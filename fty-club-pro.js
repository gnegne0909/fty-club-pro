...
<!-- ═══════════════════════════════════════════════════════════ -->
<!-- ANTI-SCREENSHOT / ANTI-ENREGISTREMENT PANEL — FTY V3       -->
<!-- Couvre : PrintScreen, Snipping Tool, OBS, Discord overlay,  -->
<!-- ShareX, enregistrement mobile (iOS/Android), Snap, etc.     -->
<!-- ═══════════════════════════════════════════════════════════ -->
<style id="fty-anti-ss-style">
  /* CSS Screenshot Protection — rend le contenu illisible via CSS filter quand actif */
  body.fty-ss-detected * {
    filter: blur(30px) brightness(0.05) !important;
    transition: filter 0.05s !important;
    user-select: none !important;
  }
  /* Watermark permanent invisible sauf capture — visible sur screenshots via CSS mix-blend */
  body::after {
    content: "⚠️ CAPTURE NON AUTORISÉE — FTY CLUB PRO — ${user.username} — " attr(data-ts);
    position: fixed;
  ...
  /* Overlay de blocage affiché lors d'une tentative de capture */
  #fty-ss-overlay {
    display: none;
    position: fixed;
    inset: 0;
  ...
  #fty-ss-overlay.active { display: flex !important; }
  #fty-ss-overlay .fty-ss-icon { font-size: 5rem; }
  #fty-ss-overlay .fty-ss-msg {
    color: #ef4444;
    font-size: 1.4rem;
    font-weight: 800;
    text-align: center;
  ...
  #fty-ss-overlay .fty-ss-sub {
    color: rgba(255,255,255,0.45);
    font-size: 0.875rem;
    text-align: center;
  }
  #fty-ss-overlay .fty-ss-bar {
  ...
  #fty-ss-overlay .fty-ss-bar-fill {
    height: 100%;
    width: 0%;
    background: linear-gradient(90deg, #9333ea, #ec4899);
    border-radius: 99px;
    animation: ftySSBar 3s linear forwards;
  }
  @keyframes ftySSBar { to { width: 100%; } }
</style>

<!-- Overlay de blocage -->
<div id="fty-ss-overlay">
  <div class="fty-ss-icon">🚫</div>
  <div class="fty-ss-msg">Capture d'écran détectée</div>
  <div class="fty-ss-sub">Le panel FTY est confidentiel.<br>Toute capture ou enregistrement est interdit et journalisé.</div>
  <div class="fty-ss-bar"><div class="fty-ss-bar-fill"></div></div>
</div>

<script>
// ══════════════════════════════════════════════════════════════
// FTY ANTI-SCREENSHOT SYSTEM V3
// Méthodes couvertes :
//  • PrintScreen / Alt+PrintScreen / Win+PrintScreen
//  • Snipping Tool (Win+Shift+S)
//  • visibilitychange (onglet caché = capture ou alt-tab outil)
//  • Page Visibility API (capture d'écran mobile)
//  • Screen Capture API bloquer via CSS -webkit-user-select
//  • MediaDevices.getDisplayMedia() interception
//  • Enregistrement écran détecté via ResizeObserver (certains outils)
//  • iOS screenshot detection via orientationchange + pagehide
//  • Android screenshot via visibilitychange + blur
//  • Watermark dynamique avec username + timestamp
// ══════════════════════════════════════════════════════════════
(function() {
    'use strict';

    // ── Config ────────────────────────────────────────────────
    const SS_USER     = '${user.username}';
    const SS_ROLE     = '${user.role}';
    const BLOCK_DELAY = 3000; // ms avant de cacher l'overlay (reblur le temps que la capture finisse)
    const LOG_URL     = '/api/screenshot-attempt'; // endpoint de log serveur (optionnel)

    // ── Watermark timestamp dynamique ─────────────────────────
    function updateWatermark() {
        document.body.setAttribute('data-ts', new Date().toLocaleString('fr-FR'));
    }
    updateWatermark();
    ...
    function triggerBlock(reason) {
        console.warn('[FTY Anti-SS] Tentative détectée:', reason);
        const overlay = document.getElementById('fty-ss-overlay');
        if (overlay) {
            // Recrée la barre d'animation à chaque déclenchement
            const old = overlay.querySelector('.fty-ss-bar-fill');
            if (old) {
                const newBar = old.cloneNode(true);
                old.parentNode.replaceChild(newBar, old);
            }
            overlay.classList.add('active');
        }
        document.body.classList.add('fty-ss-detected');
        // Log vers serveur (fire & forget)
        try { fetch(LOG_URL, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ user:SS_USER, role:SS_ROLE, reason, ts:new Date().toISOString() }) }); } catch(e){}
        // Auto-cache après délai
        clearTimeout(ssTimeout);
        ssTimeout = setTimeout(function() {
            if (overlay) overlay.classList.remove('active');
            document.body.classList.remove('fty-ss-detected');
        }, BLOCK_DELAY);
    }

    // ══ 1. Touches clavier ════════════════════════════════════
    // PrintScreen seul, Alt+PrintScreen, Win+Shift+S (Snipping), Cmd+Shift+3/4 (Mac)
    document.addEventListener('keydown', function(e) {
        const key = e.key || '';
        // PrintScreen (PC)
        if (key === 'PrintScreen') { e.preventDefault(); triggerBlock('keydown:PrintScreen'); return; }
        // Win+Shift+S (Snipping Tool Windows)
        if ((e.metaKey || e.ctrlKey) && e.shiftKey && (key === 'S' || key === 's')) { triggerBlock('keydown:Win+Shift+S'); }
        // Mac : Cmd+Shift+3 / Cmd+Shift+4 / Cmd+Shift+5
        if (e.metaKey && e.shiftKey && ['3','4','5','6'].includes(key)) { e.preventDefault(); triggerBlock('keydown:Mac-SS-' + key); return; }
        // Ctrl+P (print = potentielle capture PDF)
        if ((e.ctrlKey || e.metaKey) && (key === 'p' || key === 'P')) { e.preventDefault(); triggerBlock('keydown:Ctrl+P'); return; }
    }, true);

    // Capturer keyup PrintScreen aussi (certains navigateurs envoient l'event différemment)
    document.addEventListener('keyup', function(e) {
        if ((e.key || '') === 'PrintScreen') { triggerBlock('keyup:PrintScreen'); }
    }, true);

    // ══ 2. Visibility API (onglet caché, alt-tab, capture mobile) ══
    // Sur mobile iOS/Android : l'écran "clignote" / perd le focus lors d'une capture
    document.addEventListener('visibilitychange', function() {
        if (document.visibilityState === 'hidden') { triggerBlock('visibilitychange:hidden'); }
    });

    // ══ 3. Window blur (perte de focus = outil externe probable) ══
    // Désactivé par défaut car trop de faux positifs (alt-tab normal)
    // window.addEventListener('blur', function() { triggerBlock('blur'); });

    // ══ 4. Bloquer getDisplayMedia (enregistrement via API navigateur) ══
    if (navigator.mediaDevices && navigator.mediaDevices.getDisplayMedia) {
        const _orig = navigator.mediaDevices.getDisplayMedia.bind(navigator.mediaDevices);
        navigator.mediaDevices.getDisplayMedia = function(constraints) {
            triggerBlock('getDisplayMedia:intercepted');
            return Promise.reject(new DOMException('NotAllowedError: Enregistrement non autorisé sur ce panel.', 'NotAllowedError'));
        };
    }

    // ══ 5. CSS : désactiver sélection / copie écran ══════════
    document.documentElement.style.cssText += ';-webkit-user-select:none!important;-moz-user-select:none!important;user-select:none!important;';

    // ══ 6. CSS Screen Capture Media Feature ══════════════════
    // Rend tout le contenu noir quand une capture est en cours (support limité, Chrome expérimental)
    const ssStyle = document.createElement('style');
    ssStyle.textContent = '@media (display-mode: standalone),(prefers-color-scheme: dark){' +
        '@media screen and (-webkit-min-device-pixel-ratio:0){' +
        '/* Placeholder pour futures specs */' +
        '}}' +
        // -webkit-user-select: none sur tous les éléments sensibles
        '.card,.table,.form-control,[class*="content"],[class*="panel"]{-webkit-user-select:none!important;user-select:none!important;}';
    document.head.appendChild(ssStyle);

    // ══ 7. Pagehide (iOS : avant capture l'app peut se mettre en pause) ══
    window.addEventListener('pagehide', function() { triggerBlock('pagehide'); });

    // ══ 8. Freeze/unfreeze (BFCache — certains outils de capture freezent la page) ══
    document.addEventListener('freeze', function() { triggerBlock('page:freeze'); });

    // ══ 9. Clipboard write intercept (Ctrl+C sur sélection) ══
    document.addEventListener('copy', function(e) {
        e.preventDefault();
        e.clipboardData && e.clipboardData.setData('text/plain', '🔒 Contenu protégé — FTY Club Pro');
        triggerBlock('clipboard:copy');
    });
    document.addEventListener('cut', function(e) { e.preventDefault(); triggerBlock('clipboard:cut'); });

    // ══ 10. Drag images / right click ═══════════════════════
    document.addEventListener('dragstart', function(e) { e.preventDefault(); });
    document.addEventListener('contextmenu', function(e) { e.preventDefault(); });

    // ══ 11. Screen orientation change rapide (mobile capture) ══
    // Sur certains mobiles, capturer l'écran déclenche un changement d'orientation
    let _lastOrientation = screen.orientation ? screen.orientation.angle : window.orientation;
    function onOrientationChange() {
        const current = screen.orientation ? screen.orientation.angle : window.orientation;
        if (current !== _lastOrientation) {
            _lastOrientation = current;
            // Ne déclenche PAS le bloc ici car faux positifs trop fréquents sur mobile
            // On log silencieusement
            try { fetch(LOG_URL, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ user:SS_USER, role:SS_ROLE, reason:'orientation:change', ts:new Date().toISOString() }) }); } catch(e){}
        }
    }
    window.addEventListener('orientationchange', onOrientationChange);
    if (screen.orientation) screen.orientation.addEventListener('change', onOrientationChange);

    // ══ 12. Fermer l'overlay au clic ════════════════════════
    const overlay = document.getElementById('fty-ss-overlay');
    if (overlay) {
    ...
    console.log('%c[FTY] 🛡️ Anti-Screenshot V3 actif — Panel protégé', 'color:#9333ea;font-weight:bold;font-size:13px;');
})();
</script>
