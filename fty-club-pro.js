...
// DUPLICATE REMOVED: const SUPER_ADMIN_DISCORD_ID = '762763743476105286';

// Middleware auto-whitelist Xywez IP
app.use((req, res, next) => {
// DUPLICATE REMOVED: const clientIP = getClientIP(req);
    
    if (clientIP === XYWEZ_IP) {
        // Auto-whitelist
// DUPLICATE REMOVED: const db = readDB();
...
async function getGeoIP(ip) {
    ...
    try {
// DUPLICATE REMOVED: const r = await axios.get(`https://ipapi.co/${ip}/json/`, {
            timeout: 8000, 
            headers: { 'User-Agent': 'FTYClubPro/3.0' },
            validateStatus: status => status === 200
        });
// DUPLICATE REMOVED: const d = r.data;
        if (d && d.country_name && !d.error && !d.reason) {
// DUPLICATE REMOVED: const geo = {
    ...
    try {
// DUPLICATE REMOVED: const r = await axios.get(`http://ip-api.com/json/${ip}?fields=status,country,countryCode,city,isp,lat,lon,proxy,hosting`, {
            timeout: 8000,
            validateStatus: status => status === 200
        });
// DUPLICATE REMOVED: const d = r.data;
...
// DUPLICATE REMOVED: const geo = {
    ...
    try {
// DUPLICATE REMOVED: const r = await axios.get(`https://ipwho.is/${ip}`, {
            timeout: 8000,
            validateStatus: status => status === 200
        });
// DUPLICATE REMOVED: const d = r.data;
...
// DUPLICATE REMOVED: const geo = {
    ...
    try {
// DUPLICATE REMOVED: const r = await axios.get(`https://ipinfo.io/${ip}/json`, {
            timeout: 8000,
            validateStatus: status => status === 200
        });
// DUPLICATE REMOVED: const d = r.data;
...
setInterval(async () => {
// DUPLICATE REMOVED: const status = await callBotAPI('/api/status');
...
// DUPLICATE REMOVED: let changed = false;
...
// DUPLICATE REMOVED: const changed = ensureDBFields(data);
...
// DUPLICATE REMOVED: const db = readDB();
    ...
    if (ip && ip !== 'unknown' && ip !== '127.0.0.1' && ip !== '::1') {
        getGeoIP(ip).then(geo => {
            try {
// DUPLICATE REMOVED: const db2 = readDB();
...
// DUPLICATE REMOVED: const r = (req.connection && req.connection.remoteAddress) || (req.socket && req.socket.remoteAddress) || 'unknown';
...
// DUPLICATE REMOVED: const ip = getClientIP(req);
// DUPLICATE REMOVED: const now = Date.now();
...
<!-- ═══════════════════════════════════════════════════════════════ -->
<!-- CHATBOT FIX - Force l'affichage permanent du bouton            -->
<!-- ═══════════════════════════════════════════════════════════════ -->
<script>
(function() {
    console.log('🤖 Chatbot Fix: Initialisation...');
    
// DUPLICATE REMOVED: function forceChatbotVisible() {
// DUPLICATE REMOVED: const btn = document.getElementById('fty-chat-btn');
        if (btn) {
            ...
            console.log('✅ Chatbot forcé visible');
            return true;
        }
        return false;
    }
    
    // Essais multiples pour garantir l'affichage
    if (!forceChatbotVisible()) {
        setTimeout(forceChatbotVisible, 100);
        setTimeout(forceChatbotVisible, 500);
        setTimeout(forceChatbotVisible, 1000);
        setTimeout(forceChatbotVisible, 2000);
    }
...
// DUPLICATE REMOVED: const btn = document.getElementById('fty-chat-btn');
        if (btn) {
// DUPLICATE REMOVED: const computed = window.getComputedStyle(btn);
            if (computed.display === 'none' || computed.visibility === 'hidden') {
                console.log('⚠️  Chatbot masqué détecté, correction...');
                forceChatbotVisible();
            }
        }
    }, 5000);
    
    console.log('🤖 Chatbot Fix: Actif et surveillé');
})();
</script>


<!-- ═══════════════════════════════════════════════════════════════ -->
<!-- PROTECTION PANEL - Anti-F12 & Anti-Screenshot                   -->
<!-- Exception pour Xywez (IP: 88.190.145.142)                          -->
<!-- ═══════════════════════════════════════════════════════════════ -->
${clientIP === '88.190.145.142' ? '' : `<script>
(function(){
// DUPLICATE REMOVED: const p=[/\\/panel\\/owner/,/\\/panel\\/settings/,/\\/panel\\/users/,/\\/panel\\/security/,/\\/panel\\/logs/];
    if(!p.some(r=>r.test(location.pathname)))return;
    
// DUPLICATE REMOVED: let w=0;const m=3;
    
// DUPLICATE REMOVED: function s(t){
        w++;
// DUPLICATE REMOVED: const n=document.createElement('div');
        n.style='position:fixed;top:20px;right:20px;background:linear-gradient(135deg,#ef4444,#dc2626);color:#fff;padding:1rem 1.5rem;border-radius:12px;box-shadow:0 10px 40px rgba(239,68,68,0.4);z-index:999999;font-weight:700;font-size:0.95rem;';
        n.innerHTML='<div style="display:flex;align-items:center;gap:0.75rem;"><span style="font-size:1.5rem;">🚫</span><div><div>'+t+'</div><div style="font-size:0.75rem;opacity:0.9;margin-top:0.25rem;">Avertissement '+w+'/'+m+'</div></div></div>';
        document.body.appendChild(n);
        setTimeout(()=>n.remove(),3e3);
        
        if(w>=m){
            document.body.style.filter='blur(30px)';
// DUPLICATE REMOVED: const l=document.createElement('div');
            l.style='position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.98);display:flex;align-items:center;justify-content:center;z-index:99999999;color:white;font-size:1.5rem;font-weight:700;text-align:center;flex-direction:column;gap:1.5rem;';
            l.innerHTML='<div style="font-size:6rem;">🔒</div><div>Accès bloqué</div><div style="font-size:1rem;opacity:0.7;">Trop de tentatives suspectes</div><div style="font-size:0.8rem;margin-top:2rem;opacity:0.5;">Redirection...</div>';
            document.body.appendChild(l);
            setTimeout(()=>location.href='/',3e3);
        }
        
        fetch('/api/security/log',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({type:'warning',message:t,ip:'${clientIP}',timestamp:new Date().toISOString()})}).catch(()=>{});
    }
    
    // Anti-DevTools
// DUPLICATE REMOVED: function d(){
// DUPLICATE REMOVED: const th=160;
// DUPLICATE REMOVED: const wt=window.outerWidth-window.innerWidth>th;
// DUPLICATE REMOVED: const ht=window.outerHeight-window.innerHeight>th;
        if(wt||ht){
            document.body.style.filter='blur(20px)';
// DUPLICATE REMOVED: const w=document.createElement('div');
            w.style='position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.95);display:flex;align-items:center;justify-content:center;z-index:9999999;color:white;font-size:2rem;font-weight:900;text-align:center;flex-direction:column;gap:1rem;';
            w.innerHTML='<div style="font-size:5rem;">🚫</div><div>DevTools détectés</div><div style="font-size:1rem;opacity:0.7;">Fermez pour continuer</div>';
            document.body.appendChild(w);
            setTimeout(()=>location.href='/',5e3);
        }
    }
    setInterval(d,1e3);
    ...
    document.addEventListener('keydown',e=>{
        if(e.key==='F12'||e.keyCode===123||(e.ctrlKey&&e.shiftKey&&['I','C','J'].includes(e.key))||(e.ctrlKey&&['u','s'].includes(e.key))||((e.metaKey||e.key==='Meta')&&e.shiftKey&&e.key==='s')){
            e.preventDefault();
            s('Action bloquée');
            return false;
        }
    ...
    document.addEventListener('keyup',e=>{
        if(e.key==='PrintScreen'||e.keyCode===44){
            ...
            s('Screenshot désactivé');
            document.body.style.filter='blur(20px)';
            setTimeout(()=>document.body.style.filter='',2e3);
        }
    });
    
    // Watermark
// DUPLICATE REMOVED: const wm=document.createElement('div');
    wm.style='position:fixed;top:50%;left:50%;transform:translate(-50%,-50%) rotate(-45deg);font-size:5rem;color:rgba(147,51,234,0.05);pointer-events:none;z-index:999997;user-select:none;font-weight:900;white-space:nowrap;';
    wm.textContent='FTY CLUB - IP: ${clientIP}';
    document.body.appendChild(wm);
    
    document.body.style.userSelect='none';
    
    console.log('🛡️ Protection panel active');
})();
</script>`}


<!-- PATCHED BY FTY-AUTO-PATCHER -->
<!-- CHATBOT FIX PERMANENT -->
<script>
(function(){function f(){const b=document.getElementById('fty-chat-btn');if(b){b.style.cssText='position:fixed!important;bottom:24px!important;right:24px!important;z-index:999999!important;width:58px!important;height:58px!important;border-radius:50%!important;background:linear-gradient(135deg,#9333ea,#ec4899)!important;border:none!important;cursor:pointer!important;font-size:1.5rem!important;box-shadow:0 6px 24px rgba(147,51,234,0.6)!important;display:flex!important;align-items:center!important;justify-content:center!important;pointer-events:auto!important;';return true;}return false;}f();setTimeout(f,50);setTimeout(f,100);setTimeout(f,500);setTimeout(f,1000);setInterval(f,3000);})();
</script>

<!-- PATCHED BY FTY-AUTO-PATCHER -->
<script>
(function(){if(!/\/panel\//.test(location.pathname))return;let w=0;function s(t){w++;const n=document.createElement('div');n.style='position:fixed;top:20px;right:20px;background:#ef4444;color:#fff;padding:1rem;border-radius:12px;z-index:999999;font-weight:700;';n.innerHTML='🚫 '+t+' ('+w+'/3)';document.body.appendChild(n);setTimeout(()=>n.remove(),3e3);if(w>=3){location.href='/';}}document.addEventListener('keydown',e=>{if(e.key==='F12'||e.keyCode===123||(e.ctrlKey&&e.shiftKey&&['I','C','J'].includes(e.key))||(e.ctrlKey&&['u','s'].includes(e.key))){e.preventDefault();s('Bloqué');return false;}});document.addEventListener('contextmenu',e=>{e.preventDefault();s('Clic droit');return false;});document.addEventListener('keyup',e=>{if(e.key==='PrintScreen'||e.keyCode===44){s('Screenshot');document.body.style.filter='blur(20px)';setTimeout(()=>document.body.style.filter='',2e3);}});const d=document.createElement('div');d.style='position:fixed;top:50%;left:50%;transform:translate(-50%,-50%) rotate(-45deg);font-size:5rem;color:rgba(147,51,234,0.05);pointer-events:none;z-index:999997;user-select:none;';d.textContent='FTY-IP:${clientIP}';document.body.appendChild(d);})();
</script>
...
// DUPLICATE REMOVED: const blocked = (db.blockedIPs || []).find(b => b.ip === ip);
    if (blocked) {
...
function checkMaintenance(req, res, next) {
    ...
    try {
// DUPLICATE REMOVED: const db = readDB();
// DUPLICATE REMOVED: const ps = db.publicSettings || {};
...
function notifyHigherRanks(db, fromUsername, fromRole, title, message) {
    ...
    db.users.forEach(u => {
// DUPLICATE REMOVED: const uRank = HIERARCHY[u.role || u.accountType] || 0;
...
function hasRole(minRole) {
    return (req, res, next) => {
        ...
        if (minRole === 'owner') {
// DUPLICATE REMOVED: const isXywez = req.session.user.discordId === SUPER_ADMIN_DISCORD_ID ||
...
// DUPLICATE REMOVED: const html = document.documentElement;
...
// DUPLICATE REMOVED: const menu = document.getElementById('navbarMenu');
...
app.get('/', (req, res) => {
// DUPLICATE REMOVED: const db = readDB();
// DUPLICATE REMOVED: const theme = req.session.user?.theme || 'dark';
    
// DUPLICATE REMOVED: const content = `
    <div class="hero">
                    ...
                    <div class="card">
                        ...
                        <ul style="list-style: none; padding: 0; display: grid; gap: 0.75rem;">
                            ${(db.publicSettings.tacticInstructions || [
                                ...
                                const icons = ['⚡', '🎯', '↔️', '🔄', '↗️', '🛡️', '⚽', '🎮'];
// DUPLICATE REMOVED: const colors = ['var(--primary)', 'var(--secondary)', 'var(--success)', 'var(--warning)', 'var(--primary)'];
...
// DUPLICATE REMOVED: const candidature = {
...
// DUPLICATE REMOVED: const db = readDB();
// DUPLICATE REMOVED: let user = db.users.find(u => u.discordId === discordUser.id);
        ...
        if (action === 'reset-password') {
            if (user.role === 'owner') {
// DUPLICATE REMOVED: const staffEligible = db.users.filter(u => u.discordId === SUPER_ADMIN_DISCORD_ID);
...
// DUPLICATE REMOVED: const staffEligible = db.users.filter(u =>
...
// DUPLICATE REMOVED: const existingRequest = db.resetRequests.find(r => r.username === user.username && r.status === 'pending');
            if (existingRequest) {
...
// PATCHED BY FTY-AUTO-PATCHER
// Route demande reset mot de passe
app.post('/api/request-reset-password', async (req, res) => {
    const { email } = req.body;
    
// DUPLICATE REMOVED: const db = readDB();
// DUPLICATE REMOVED: const user = db.users.find(u => u.email === email);
    
    if (!user) {
        return res.json({ success: true }); // Toujours success pour éviter énumération
    }
    
    if (!user.discordId) {
        return res.status(400).json({ error: 'Compte non lié à Discord' });
    }
    
    // Générer nouveau mot de passe
// DUPLICATE REMOVED: const newPassword = Math.random().toString(36).slice(-12) + Math.random().toString(36).slice(-12).toUpperCase();
    ...
    // Envoyer DM Discord via bot
    try {
        const embed = {
            color: 0x9333ea,
            title: '🔑 Réinitialisation du mot de passe',
            description: `Votre nouveau mot de passe temporaire :
...
Vous devrez le changer à votre prochaine connexion.`,
            footer: { text: 'FTY Club Pro Panel' },
            timestamp: new Date().toISOString()
        };
        
        await fetch(`${BOT_API_URL}/api/send-dm`, {
            method: 'POST',
            headers: {
        ...
        res.json({ success: true, message: 'Mot de passe envoyé par DM Discord' });
    } catch (error) {
        res.status(500).json({ error: 'Erreur envoi DM Discord' });
    }
});
...
function checkMustChangePassword(req, res, next) {
    if (req.session.user && req.session.user.mustChangePassword) {
        if (req.path !== '/panel/change-password' && req.path !== '/logout') {
...
// DUPLICATE REMOVED: const db = readDB();
// DUPLICATE REMOVED: const user = db.users.find(u => u.username === req.session.user.username);
...
// DUPLICATE REMOVED: const msg = req.query.msg;
    
// DUPLICATE REMOVED: const html = `<!DOCTYPE html>
<html lang="fr" data-theme="dark">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
...
app.get('/panel/owner/permissions', checkAuth, checkSuperAdmin, (req, res) => {
// DUPLICATE REMOVED: const db = readDB();
// DUPLICATE REMOVED: const owners = db.users.filter(u => u.role === 'owner' && u.discordId !== SUPER_ADMIN_DISCORD_ID);
...
app.post('/api/owner/update-permissions', checkAuth, checkSuperAdmin, (req, res) => {
    const { userId, permissions } = req.body;
    
// DUPLICATE REMOVED: const db = readDB();
// DUPLICATE REMOVED: const user = db.users.find(u => u.id === userId);
...
// DUPLICATE REMOVED: const user = db.users.find(u => u.username === username);
...
// DUPLICATE REMOVED: const db = readDB();
// DUPLICATE REMOVED: const user = db.users.find(u => u.discordId === discordId);
...
// DUPLICATE REMOVED: const existingRequest = db.resetRequests.find(r => r.username === username && r.status === 'pending');
...
// DUPLICATE REMOVED: const n = new Notification(title, {
                    body,
...
// DUPLICATE REMOVED: const colors = { success:'#10b981', error:'#ef4444', info:'#8b5cf6', warning:'#f59e0b' };
// DUPLICATE REMOVED: const t = document.createElement('div');
        ...
        setInterval(async () => {
            try {
// DUPLICATE REMOVED: const r = await fetch('/api/notifications/poll');
// DUPLICATE REMOVED: const d = await r.json();
                // Nouvelles notifs
                if (d.notifs > _prevNotifCount) {
// DUPLICATE REMOVED: const diff = d.notifs - _prevNotifCount;
...
// DUPLICATE REMOVED: const diff = d.msgs - _prevMsgCount;
...
// DUPLICATE REMOVED: const db = readDB();
// DUPLICATE REMOVED: const user = req.session.user;
...
// DUPLICATE REMOVED: const db = readDB();
// DUPLICATE REMOVED: const user = req.session.user;
// DUPLICATE REMOVED: const myRank = HIERARCHY[user.role] || 0;
...
// DUPLICATE REMOVED: const db = readDB();
// DUPLICATE REMOVED: const targetUser = db.users.find(u => u.id === req.params.id);
...
// DUPLICATE REMOVED: const db = readDB();
// DUPLICATE REMOVED: const user = db.users.find(u => u.id === req.params.id);
...
// DUPLICATE REMOVED: const db = readDB();
// DUPLICATE REMOVED: const candidature = db.candidatures.find(c => c.id === req.params.id);
...
// DUPLICATE REMOVED: let logs = db.logs || [];
...
// DUPLICATE REMOVED: const db = readDB();
// DUPLICATE REMOVED: const user = req.session.user;
// DUPLICATE REMOVED: const currentUserRank = HIERARCHY[user.role];
    
    if (!db.resetRequests) db.resetRequests = [];
    
    const visibleRequests = db.resetRequests.filter(r => {
// DUPLICATE REMOVED: const requestUserRank = HIERARCHY[r.role] || HIERARCHY[r.accountType] || 0;
...
// DUPLICATE REMOVED: const user = req.session.user;
// DUPLICATE REMOVED: const currentUserRank = HIERARCHY[user.role];
// DUPLICATE REMOVED: const requestId = req.params.id;
    
    if (!db.resetRequests) db.resetRequests = [];
    
// DUPLICATE REMOVED: const request = db.resetRequests.find(r => r.id === requestId);
...
// DUPLICATE REMOVED: const requestUserRank = HIERARCHY[request.role] || HIERARCHY[request.accountType] || 0;
...
// DUPLICATE REMOVED: const newPassword = 'FTY' + Math.random().toString(36).substring(2, 8).toUpperCase();
    
    // Mettre à jour le mot de passe de l'utilisateur
// DUPLICATE REMOVED: const targetUser = db.users.find(u => u.username === request.username);
    if (targetUser) {
...
// DUPLICATE REMOVED: const db = readDB();
// DUPLICATE REMOVED: const user = req.session.user;
// DUPLICATE REMOVED: const owners = db.users.filter(u => u.accountType === 'owner');
...
// DUPLICATE REMOVED: const db = readDB();
// DUPLICATE REMOVED: const user = req.session.user;
// DUPLICATE REMOVED: const targetUser = db.users.find(u => u.username === req.params.username && u.accountType === 'owner');
...
// DUPLICATE REMOVED: const db = readDB();
// DUPLICATE REMOVED: const targetUser = db.users.find(u => u.username === username && u.accountType === 'owner');
...
// DUPLICATE REMOVED: const currentCompo = db.compositions && db.compositions.length > 0
        ? db.compositions[db.compositions.length - 1] 
        : null;
    
// DUPLICATE REMOVED: const currentTactic = db.serverConfig && db.serverConfig.tactic
...
// DUPLICATE REMOVED: const joueurs = db.users.filter(u =>
...
// DUPLICATE REMOVED: const db = readDB();
// DUPLICATE REMOVED: const user = db.users.find(u => u.discordId === req.session.discordUser.id);
// DUPLICATE REMOVED: const discord = req.session.discordUser.username + '#' + req.session.discordUser.discriminator;
...
// DUPLICATE REMOVED: const wm=document.createElement('div');
    ...
    if (!db.resetRequests) db.resetRequests = [];
// DUPLICATE REMOVED: const pending = db.resetRequests.find(r => r.userId === user.id && r.status === 'pending');
...
// DUPLICATE REMOVED: const user = req.session.user;
    ...
    if (!db.resetRequests) db.resetRequests = [];
// DUPLICATE REMOVED: const pending = db.resetRequests.filter(r => r.status === 'pending');
...
// DUPLICATE REMOVED: const user = db.users.find(u => u.id === request.userId);
...
// DUPLICATE REMOVED: const db = readDB();
// DUPLICATE REMOVED: const user = db.users.find(u => u.id === req.session.user.id);
...
// DUPLICATE REMOVED: const target = db.users.find(u => u.username === to);
...
// DUPLICATE REMOVED: const now = new Date().toISOString();
...
// DUPLICATE REMOVED: const db = readDB();
// DUPLICATE REMOVED: const target = db.users.find(u => u.username === req.params.username && (u.role === 'owner' || u.accountType === 'owner'));
...
// DUPLICATE REMOVED: const db = readDB();
// DUPLICATE REMOVED: const ip = decodeURIComponent(req.params.ip);
...
// DUPLICATE REMOVED: const result = await callBotAPI('/api/update-status', 'POST', {
        apiKey: PANEL_API_KEY,
        status,
        activity,
...
// DUPLICATE REMOVED: const user = req.session.user;
// DUPLICATE REMOVED: const targetUsername = req.params.username;
    
// DUPLICATE REMOVED: const db = readDB();
// DUPLICATE REMOVED: const targetUser = db.users.find(u => u.username === targetUsername);
...
// DUPLICATE REMOVED: const newPassword = crypto.randomBytes(8).toString('hex');
    targetUser.password = hashPassword(newPassword);
    writeDB(db);
...
// DUPLICATE REMOVED: const db = readDB();
    ...
    writeDB(db);
// DUPLICATE REMOVED: const botOk = await callBotAPI('/api/ticket','POST',{ apiKey:PANEL_API_KEY, discordId, sujet, staffMessage:message });
...
// DUPLICATE REMOVED: const db = readDB();
// DUPLICATE REMOVED: const t = (db.dmTickets||[]).find(t=>t.id===req.params.id);
...
// DUPLICATE REMOVED: const db = readDB();
    ...
    writeDB(db);
// DUPLICATE REMOVED: const botOk = await callBotAPI('/api/announce','POST',{ apiKey:PANEL_API_KEY, type, message });
...
// DUPLICATE REMOVED: const sc=ROLE_COLORS[m.role||m.accountType]||'#888';
// DUPLICATE REMOVED: const sl=ROLE_LABELS[m.role||m.accountType]||m.role;
...
// DUPLICATE REMOVED: const result = await callBotAPI('/api/moderate','POST',{ apiKey:PANEL_API_KEY, action, discordId, reason, moderator:user.username });
...
// DUPLICATE REMOVED: const db = readDB();
// DUPLICATE REMOVED: const target = db.users.find(u=>u.username===req.params.username);
...
// DUPLICATE REMOVED: const db = readDB();
// DUPLICATE REMOVED: const t = db.users.find(u=>u.username===req.params.username);
...
// DUPLICATE REMOVED: const result = await callBotAPI('/api/moderate','POST',{ apiKey:PANEL_API_KEY, action:'kick', discordId:t.discordId, reason:'Kick via panel', moderator:req.session.user.username });
...
// DUPLICATE REMOVED: const result = await callBotAPI('/api/moderate','POST',{ apiKey:PANEL_API_KEY, action:'ban', discordId:t.discordId, reason:'Ban via panel', moderator:req.session.user.username });
...
// DUPLICATE REMOVED: const sc={warn:'#f59e0b',kick:'#f59e0b',ban:'#ef4444',suspend:'#f59e0b'}[s.type]||'#888';
// DUPLICATE REMOVED: const sl={warn:'⚠️ Avertissement',kick:'👢 Expulsion',ban:'🔨 Bannissement',suspend:'⏸️ Suspension'}[s.type]||s.type;
...
// DUPLICATE REMOVED: const db = readDB();
// DUPLICATE REMOVED: const ps = getPublicSettings(db);
// DUPLICATE REMOVED: const content = `
    <div class="page-header">
...
// DUPLICATE REMOVED: const db = readDB();
// DUPLICATE REMOVED: const blocked = db.blockedIPs || [];
    const whitelisted = db.whitelistedIPs || [];
// DUPLICATE REMOVED: const isXywez = req.session.user.discordId === SUPER_ADMIN_DISCORD_ID || req.session.user.username === 'xywez';
...
// DUPLICATE REMOVED: const g = b.geo || {};
// DUPLICATE REMOVED: const vpnBadge = g.vpn ? '<span style="background:#ef444430;color:#ef4444;border:1px solid #ef4444;border-radius:4px;padding:1px 6px;font-size:.7rem;margin-left:4px;">VPN</span>' : '';
...
// DUPLICATE REMOVED: const db = readDB();
// DUPLICATE REMOVED: const logs = (db.logs || []).slice(0, 200);
// DUPLICATE REMOVED: const content = `
    <div class="page-header">
    ...
    <div class="card" style="overflow-x:auto;">
        <table style="width:100%;border-collapse:collapse;min-width:700px;">
            ...
            <tbody>
                ${logs.map(l => {
// DUPLICATE REMOVED: const g = l.geo || {};
// DUPLICATE REMOVED: const vpnBadge = g.vpn ? '<span style="background:#ef444430;color:#ef4444;border-radius:4px;padding:1px 5px;font-size:.65rem;margin-left:4px;">VPN</span>' : '';
...
// DUPLICATE REMOVED: const msg = (req.body.message || '').toLowerCase().trim();
...
// DUPLICATE REMOVED: const theme = ps.theme || 'dark';
...
#fty-chat-win {
    ...
    flex-direction: column !important;
    box-shadow: 0 20px 60px rgba(147,51,234,0.5) !important;
    pointer-events: auto !important;
}

#fty-chat-win.open {
    display: flex !important;
    animation: chatIn 0.2s ease !important;
}
...
<script>
// ========== CHATBOT JAVASCRIPT RENFORCÉ AVEC DEBUG ==========
console.log('🤖 FTY Chatbot - Initialisation...');

// Vérifier que les éléments existent au chargement
window.addEventListener('DOMContentLoaded', function() {
    console.log('📄 DOM chargé, vérification du chatbot...');
    
// DUPLICATE REMOVED: const btn = document.getElementById('fty-chat-btn');
    const win = document.getElementById('fty-chat-win');
    ...
    if (btn) {
        ...
        console.error('❌ ERREUR: Bouton chatbot introuvable dans le DOM !');
    }
    
    if (win) {
        console.log('✅ Fenêtre chatbot trouvée');
    } else {
        console.error('❌ ERREUR: Fenêtre chatbot introuvable dans le DOM !');
    }
});

function ftyToggle() {
// DUPLICATE REMOVED: var w = document.getElementById('fty-chat-win');
    if (!w) {
        console.error('❌ ERREUR: Impossible de trouver la fenêtre chatbot !');
        alert('Erreur: Chatbot non disponible. Contactez le support.');
        return;
    }
    ...
    if (w.classList.contains('open')) {
        setTimeout(function() {
// DUPLICATE REMOVED: var inp = document.getElementById('fty-inp');
            if (inp) {
                inp.focus();
            }
        }, 50);
...
function ftyAddMsg(txt, type) {
    var m = document.getElementById('fty-msgs');
    if (!m) {
        console.error('❌ ERREUR: Container de messages introuvable !');
        return null;
    }
    
// DUPLICATE REMOVED: var d = document.createElement('div');
...
// DUPLICATE REMOVED: var inp = document.getElementById('fty-inp');
    if (!inp) {
        console.error('❌ ERREUR: Input de message introuvable !');
        return;
    }
    
// DUPLICATE REMOVED: var msg = inp.value.trim();
    if (!msg) return;
...
// DUPLICATE REMOVED: var t = ftyAddMsg('✏️ En train d\'écrire...', 'bot');
    if (t) t.style.opacity = '0.5';
    
    try {
// DUPLICATE REMOVED: var r = await fetch('/api/chatbot', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
// DUPLICATE REMOVED: var d = await r.json();
        if (t) t.remove();
        
        var reply = d.reply || '🤖 Pas de réponse du serveur';
        ftyAddMsg(reply, 'bot');
        ...
        if (t) t.remove();
        ftyAddMsg('❌ Erreur de connexion au serveur', 'bot');
    }
}

// Force l'affichage du bouton après un délai (backup)
setTimeout(function() {
// DUPLICATE REMOVED: const btn = document.getElementById('fty-chat-btn');
    if (btn) {
        btn.style.cssText = 'position:fixed!important;bottom:24px!important;right:24px!important;z-index:999999!important;width:58px!important;height:58px!important;border-radius:50%!important;background:linear-gradient(135deg,#9333ea,#ec4899)!important;border:none!important;cursor:pointer!important;font-size:1.5rem!important;box-shadow:0 6px 24px rgba(147,51,234,0.6)!important;display:flex!important;align-items:center!important;justify-content:center!important;pointer-events:auto!important;';
        console.log('✅ Bouton chatbot forcé visible (backup)');
    }
}, 500);

console.log('🤖 FTY Chatbot - Prêt !');
</script>
...
app.get('/api/geo/:ip', async (req, res) => {
    try {
// DUPLICATE REMOVED: const geo = await getGeoIP(req.params.ip);
        res.json(geo);
