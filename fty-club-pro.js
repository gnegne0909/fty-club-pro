...
app.get('/panel/bot-config', isAuthenticated, hasRole('owner'), (req, res) => {
    // Double vérification: hasRole('owner') + check ID Discord
    if (req.session.user.discordId !== SUPER_ADMIN_DISCORD_ID) {
        return res.status(403).send(errorPage('Accès Refusé', '❌ Cette section est réservée à xywez uniquement.'));
    }
    const user = req.session.user;
...
app.get('/panel/system/backup', isAuthenticated, hasRole('owner'), (req, res) => {
    if (req.session.user.discordId !== SUPER_ADMIN_DISCORD_ID) return res.status(403).send('❌ Accès refusé - Réservé à xywez');
    const db = readDB();
