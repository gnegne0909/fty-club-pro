...
function checkMustChangePassword(req, res, next) {
    ...
    // On exclut toutes les routes nécessaires pour éviter toute boucle infinie
    const path = req.path.split('?')[0];
    const allowed = [
        '/panel/change-password', '/panel/change-password/',
        '/change-password', '/change-password/',
        '/panel/logout', '/logout'
    ];
    if (allowed.includes(path)) return next();
