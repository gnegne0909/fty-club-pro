...
// ============================================================
// ===   ANTI-4G/5G/MOBILE MODULE                          ===
// ============================================================

// Mots-clés ISP identifiant les opérateurs mobiles (4G/5G/LTE)
const MOBILE_ISP_KEYWORDS = /orange|sfr|bouygues|free mobile|iliad|altice|t-mobile|verizon wireless|at&t wireless|sprint|cricket|metro|boost mobile|vodafone|o2|three|ee limited|wind|fastweb mobile|tim|iliad|play|plus gsm|a1 telecom|proximus|base|telenet mobile|tele2|telenor|telia|elisa|dna|sunrise|swisscom|magenta|turkcell|turk telekom|avea|mobilis|djezzy|ooredoo|zain|stc|etisalat|du telecom|mtn|airtel|glo|9mobile|safaricom|m-tel|optus|telstra mobile|spark nz|2degrees|singtel|maxis|celcom|digi|globe|smart|pldt|dtac|ais|true move|xl axiata|indosat|telkomsel|reliance jio|airtel india|vi |vodafone in|bsnl|mtnl|claro|entel|movistar|personal|nextel|tim brasil|vivo|oi |claro|tigo|digicel|lime|flow|natcom/i;

// Cache anti-mobile (2h comme VPN)
const MOBILE_CACHE = {};

async function checkMobileCarrier(ip) {
    if (!ip || ip === '127.0.0.1' || ip === '::1' || ip === XYWEZ_IP ||
        ip.startsWith('192.168') || ip.startsWith('10.') || ip.startsWith('172.'))
        return { isMobile: false, carrier: '', connectionType: '' };

    // Cache 2h
    if (MOBILE_CACHE[ip] && Date.now() - MOBILE_CACHE[ip].ts < 7200000) return MOBILE_CACHE[ip].data;

    // API 1 — ip-api.com avec champ mobile
    try {
        const r = await axios.get(`http://ip-api.com/json/${ip}?fields=status,isp,org,as,mobile,query`, { timeout: 4000 });
        const d = r.data;
        if (d && d.status === 'success') {
            const ispMobile = MOBILE_ISP_KEYWORDS.test(d.isp || '') || MOBILE_ISP_KEYWORDS.test(d.org || '') || MOBILE_ISP_KEYWORDS.test(d.as || '');
            const isMobile = d.mobile === true || ispMobile;
            const result = {
                isMobile,
                carrier: d.isp || d.org || '',
                connectionType: d.mobile ? '4G/5G Mobile' : (ispMobile ? 'Mobile Carrier' : ''),
                raw_mobile: d.mobile
            };
            MOBILE_CACHE[ip] = { data: result, ts: Date.now() };
            return result;
        }
    } catch(e) {}

    // API 2 — ipwho.is (fallback, contient le champ type)
    try {
        const r2 = await axios.get(`https://ipwho.is/${ip}`, { timeout: 4000 });
        const d2 = r2.data;
        if (d2 && d2.success !== false) {
            const conn = d2.connection || {};
            const isp = conn.isp || conn.org || '';
            const connType = (d2.type || '').toLowerCase();
            const isMobile = connType.includes('mobile') || connType.includes('lte') || connType.includes('4g') || connType.includes('5g') || MOBILE_ISP_KEYWORDS.test(isp);
            const result = { isMobile, carrier: isp, connectionType: isMobile ? (connType || 'Mobile') : '' };
            MOBILE_CACHE[ip] = { data: result, ts: Date.now() };
            return result;
        }
    } catch(e) {}
...
async function checkVPNProxy(ip) {
    if (!ip || ip === '127.0.0.1' || ip === '::1' || ip === XYWEZ_IP ||
        ip.startsWith('192.168') || ip.startsWith('10.') || ip.startsWith('172.')) 
        return { isVPN: false, isProxy: false, isHosting: false, isMobile: false };
    
    // Cache 2h
    if (VPN_CACHE[ip] && Date.now() - VPN_CACHE[ip].ts < 7200000) return VPN_CACHE[ip].data;
    
    const VPN_KEYWORDS = /vpn|tunnel|proxy|hosting|cloud|server|datacenter|amazon|digitalocean|linode|vultr|hetzner|ovh|vps|dedicated|colocation|tor|exit.*node|relay|anonymi/i;
    
    // API 1 — ip-api.com (plus fiable, gratuit)
    try {
        const r = await axios.get(`http://ip-api.com/json/${ip}?fields=status,proxy,hosting,isp,org,as,mobile,query`, { timeout: 4000 });
        const d = r.data;
        if (d && d.status === 'success') {
            const isVPN = d.proxy || d.hosting || VPN_KEYWORDS.test(d.isp || '') || VPN_KEYWORDS.test(d.org || '') || VPN_KEYWORDS.test(d.as || '');
            const ispMobile = MOBILE_ISP_KEYWORDS.test(d.isp || '') || MOBILE_ISP_KEYWORDS.test(d.org || '') || MOBILE_ISP_KEYWORDS.test(d.as || '');
            const isMobile = d.mobile === true || ispMobile;
            const result = { isVPN, isProxy: d.proxy || false, isHosting: d.hosting || false, isp: d.isp || '', mobile: d.mobile || false, isMobile, mobileCarrier: isMobile ? (d.isp || '') : '' };
            VPN_CACHE[ip] = { data: result, ts: Date.now() };
            return result;
        }
    } catch(e) {}
    ...
    try {
        ...
        if (d2 && d2.success !== false) {
            ...
            const isp = conn.isp || conn.org || '';
            const connType = (d2.type || '').toLowerCase();
            const isVPN = VPN_KEYWORDS.test(isp);
            const isMobile = connType.includes('mobile') || connType.includes('lte') || connType.includes('4g') || connType.includes('5g') || MOBILE_ISP_KEYWORDS.test(isp);
            const result = { isVPN, isProxy: isVPN, isHosting: false, isp, isMobile, mobileCarrier: isMobile ? isp : '' };
            VPN_CACHE[ip] = { data: result, ts: Date.now() };
            return result;
        }
    } catch(e) {}
...
// Middleware Anti-VPN + Anti-4G/5G/Mobile renforcé
async function antiVPN(req, res, next) {
    if (req.path.startsWith('/panel') || req.path.startsWith('/auth') || req.path.startsWith('/api') || req.isXywez) return next();
    ...
    try {
        ...
        // Bloquer VPN/Proxy/Hosting si activé
        if (ps.antiVPNEnabled && (vpnCheck.isVPN || vpnCheck.isProxy || vpnCheck.isHosting)) {
            const reason = vpnCheck.isHosting ? 'Datacenter / Hosting Détecté' : vpnCheck.isProxy ? 'Proxy Détecté' : 'VPN Détecté';
            console.warn(`[AntiVPN] Bloqué: ${ip} — ${reason} (${vpnCheck.isp})`);
            return res.status(403).send(vpnBlockPage(reason));
        }
        // Bloquer 4G/5G/Mobile si activé
        if (ps.antiMobileEnabled && vpnCheck.isMobile) {
            console.warn(`[Anti4G5G] Bloqué: ${ip} — Mobile/4G/5G (${vpnCheck.mobileCarrier})`);
            addLog('📵 Anti-Mobile Bloqué', ip, req.path, { carrier: vpnCheck.mobileCarrier }, ip);
            return res.status(403).send(mobileBlockPage(vpnCheck.mobileCarrier, ps.antiMobileMessage));
        }
    } catch(e) {}
...
app.post('/panel/login', loginRateLimiter, (req, res) => {
    ...
    req.session.user = {
        ...
        firstLoginGuide: !user.hasSeenGuide // flag pour le guide
    };
    if (user.mustChangePassword) return res.redirect('/panel/change-password');
    if (!user.hasSeenGuide) return res.redirect('/panel/welcome-guide');
    ...
    <div class="card">
        ...
        <div style="display:flex;align-items:center;gap:.75rem;background:${db.publicSettings&&db.publicSettings.antiVPNEnabled?'rgba(239,68,68,.1)':'rgba(255,255,255,.04)'};border:1.5px solid ${db.publicSettings&&db.publicSettings.antiVPNEnabled?'rgba(239,68,68,.4)':'rgba(255,255,255,.08)'};border-radius:12px;padding:.875rem 1.25rem;margin-bottom:1rem;">
            ...
            <div>
                ...
                <div style="font-size:.75rem;color:var(--text-muted);">IPs en cache : ${Object.keys(VPN_CACHE).length} · Mise à jour : 2h</div>
            </div>
        </div>
...
// ── Chatbot API ───────────────────────────────────────────
// ============================================================
// ===   CHATBOT FTY ULTRA V3 — IA CONTEXTUELLE           ===
// ============================================================
...
// ============================================================
// ===   GUIDE OBLIGATOIRE — PREMIERE CONNEXION            ===
// ============================================================
...
function checkQuiz(){
  ...
  if(!q1||!q2){fb.style.display='block';fb.innerHTML='<div style="background:rgba(245,158,11,.1);border:1px solid rgba(245,158,11,.3);border-radius:10px;padding:.75rem 1rem;color:#fcd34d;font-size:.875rem;">⚠️ Réponds aux 2 questions avant de continuer.</div>';return;}
  var ok1=(q1.value==='b');var ok2=(q2.value==='a');
  ...
  document.querySelectorAll('input[name="q1"]').forEach(function(el){
    if(el.value===q1.value)el.closest('.quiz-opt').classList.add(ok1?'correct':'wrong');
  });
  document.querySelectorAll('input[name="q2"]').forEach(function(el){
    if(el.value===q2.value)el.closest('.quiz-opt').classList.add(ok2?'correct':'wrong');
  });
  if(ok1&&ok2){
    fb.style.display='block';fb.innerHTML='<div style="background:rgba(16,185,129,.1);border:1px solid rgba(16,185,129,.3);border-radius:10px;padding:.875rem 1rem;color:#6ee7b7;font-size:.875rem;text-align:center;">✅ Parfait ! Tu as tout bon. Passe à l\'étape suivante !</div>';
    setTimeout(function(){goTo(5);},1000);
  } else {
    fb.style.display='block';fb.innerHTML='<div style="background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.3);border-radius:10px;padding:.875rem 1rem;color:#fca5a5;font-size:.875rem;">❌ Mauvaise(s) réponse(s). Relis les règles à l\'étape 3 et réessaie !</div>';
  }
}
...
// ============================================================
// ===   BROADCAST STAFF ULTRA — MESSAGES IMPORTANTS       ===
// ===   Accessible: owners + fondateurs + admins + mods   ===
// ============================================================
