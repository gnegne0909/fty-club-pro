...
function initDB() {
    ...
    // Migration : s'assurer que tous les users ont role ET accountType synchronisés
    const dbMig = readDB();
    let migrated = false;
    dbMig.users.forEach(u => {
        if (!u.role) { u.role = u.accountType || 'joueur'; migrated = true; }
        if (!u.accountType) { u.accountType = u.role || 'joueur'; migrated = true; }
    });
    if (migrated) writeDB(dbMig);
}
...
function addLog(action, executor, target, details = {}, ip = '', clientInfo = {}) {
    const db = readDB();
    if (!db.logs) db.logs = [];
    db.logs.unshift({ 
...
function getClientIP(req) {
    // x-forwarded-for peut contenir "ip1, ip2, ip3" — on prend la première (client réel)
    const forwarded = req.headers['x-forwarded-for'];
    if (forwarded) {
        const first = forwarded.split(',')[0].trim();
        // Convertir ::ffff:x.x.x.x en IPv4 pur
    ...
    return remote.replace('::ffff:', '').replace('::1', '127.0.0.1 (local)');
}
...
app.get('/', (req, res) => {
    ...
    addLog('🌐 Visite accueil', req.session.user?.username || 'visiteur', '/', {}, getClientIP(req), getClientInfo(req));
...
app.post('/candidature', (req, res) => {
    ...
    writeDB(db);
    addLog('📝 Nouvelle candidature', req.body.name || 'Anonyme', req.body.position || '?', {}, getClientIP(req), getClientInfo(req));
...
app.post('/panel/login', (req, res) => {
    ...
    if (!user || !comparePassword(password, user.password)) {
        addLog('❌ Tentative connexion échouée', username || '?', 'panel/login', {}, getClientIP(req), getClientInfo(req));
    ...
    writeDB(db);
    addLog('🔐 Connexion panel', username, username, {}, clientIP, clientInfo);
...
function panelLayout(user, pageTitle, content, activePage = '') {
    ...
    if (HIERARCHY[user.role] >= HIERARCHY['moderateur']) {
        menuItems.push({
            icon: '👥',
            label: 'Membres',
            href: '/panel/users',
    ...
    if (user.role === 'capitaine' || HIERARCHY[user.role] >= HIERARCHY['manager']) {
        menuItems.push({
            icon: '🎯',
            ...
            ${(db.logs || []).slice(0, 5).map(log => `
                <div style="padding: 0.75rem 0; border-bottom: 1px solid var(--border);">
                    ...
                    <div style="font-size: 0.875rem; color: var(--text-muted);">
                        ...
                        ${log.ip ? `<span style="margin-left:6px; font-size:0.75rem; opacity:0.7;">📍 ${log.ip}</span>` : ''}
                        ${log.device ? `<span style="margin-left:4px; font-size:0.75rem; opacity:0.7;">${log.device}</span>` : ''}
                    </div>
                </div>
            `).join('')}
        </div>
...
app.post('/panel/users/create', isAuthenticated, hasRole('manager'), (req, res) => {
    ...
    if (req.session.user.username !== 'xywez' && HIERARCHY[accountType] >= HIERARCHY[req.session.user.role]) {
...
app.get('/panel/users/:id/edit', isAuthenticated, hasRole('manager'), (req, res) => {
    ...
    res.send(panelLayout(`
        <h1>Modifier ${user.username}</h1>
        <form method="POST" action="/panel/users/${user.id}/update">
            <div class="form-group">
                <label>Rôle</label>
            ...
            <div class="form-group">
                ...
                <input type="text" name="discordId" class="form-control" value="${user.discordId || ''}" placeholder="Ex: 123456789">
            </div>
            <div class="form-group">
                <label>Statut</label>
                <select name="status" class="form-control">
                    <option value="active" ${!user.suspended && !user.banned ? 'selected' : ''}>Actif</option>
                    <option value="suspended" ${user.suspended ? 'selected' : ''}>Suspendu</option>
                    <option value="banned" ${user.banned ? 'selected' : ''}>Banni</option>
                </select>
            </div>
...
app.get('/panel/logs', isAuthenticated, hasRole('administrateur'), (req, res) => {
    ...
    if (filter) {
        logs = logs.filter(l => 
            ...
            (l.executor || '').toLowerCase().includes(filter.toLowerCase()) ||
    ...
    <div class="page-header">
        <div>
            ...
            <div class="page-breadcrumb">Panel · Logs · ${(db.logs||[]).length} entrées</div>
        </div>
    ...
    <div class="card" style="margin-bottom: 1rem; padding: 1rem;">
        <form method="GET" action="/panel/logs" style="display: flex; gap: 0.75rem; align-items: center;">
            <input type="hidden" name="limit" value="${limit}">
            <input type="text" name="filter" value="${filter}" placeholder="🔍 Filtrer par action, user, IP..." 
    ...
    <div class="card" style="overflow-x: auto;">
        <div style="margin-bottom: 1rem; color: var(--text-muted); font-size: 0.85rem;">
            ${filter ? `Résultats filtrés: ${logs.length}` : `Total: ${(db.logs||[]).length} logs`} · Affichage: ${Math.min(limit, logs.length)}
        </div>
        <table class="table" style="min-width: 900px;">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="font-weight: 600;">${log.executor || '-'}</td>
                    <td>${log.target || '-'}</td>
                    <td>
                        ${log.ip ? `<code style="font-size: 0.78rem; background: var(--bg-tertiary); padding: 2px 6px; border-radius: 4px; white-space: nowrap;">${log.ip}</code>` : '<span style="color:var(--text-muted)">-</span>'}
                    </td>
                    <td style="font-size: 0.85rem; white-space: nowrap;">${log.device || '-'}</td>
                    <td style="font-size: 0.85rem;">${log.browser || '-'}</td>
                    <td style="font-size: 0.85rem; white-space: nowrap;">${log.os || '-'}</td>
                </tr>
                `).join('')}
            </tbody>
        </table>
...
app.get('/panel/reset-requests/approve/:id', isAuthenticated, hasRole('support'), (req, res) => {
    ...
    writeDB(db);
    addLog('✅ Reset MDP approuvé', user.username, request.username, { 
        targetUser: request.username,
        newPassword: newPassword 
    }, getClientIP(req), getClientInfo(req));
...
app.get('/panel/reset-requests/reject/:id', isAuthenticated, hasRole('support'), (req, res) => {
    ...
    writeDB(db);
    addLog('❌ Reset MDP rejeté', user.username, request.username, { 
        targetUser: request.username 
    }, getClientIP(req), getClientInfo(req));
...
app.get('/panel/logout', (req, res) => {
    if (req.session.user) {
        addLog('🚪 Déconnexion panel', req.session.user.username, req.session.user.username, {}, getClientIP(req), getClientInfo(req));
    }
    req.session.destroy();
    res.redirect('/panel/login');
});
