...
<!-- ═══════════════════════════════════════════════════════ -->
<!-- CHATBOT FTY - STANDALONE (styles inline, zero dépendance) -->
<!-- ═══════════════════════════════════════════════════════ -->
<button id="fty-chat-btn" onclick="ftyToggle()" title="Discuter avec FTY Assistant" style="position:fixed!important;bottom:24px!important;right:24px!important;z-index:2147483647!important;width:58px!important;height:58px!important;border-radius:50%!important;background:linear-gradient(135deg,#9333ea,#ec4899)!important;border:none!important;cursor:pointer!important;font-size:1.6rem!important;box-shadow:0 6px 28px rgba(147,51,234,.7),0 2px 8px rgba(0,0,0,.5)!important;display:flex!important;align-items:center!important;justify-content:center!important;pointer-events:auto!important;outline:none!important;transition:transform .2s!important;text-decoration:none!important;">💬</button>

<div id="fty-chat-win" style="position:fixed!important;bottom:94px!important;right:24px!important;z-index:2147483646!important;width:320px!important;height:440px!important;background:#0a0014!important;border:2px solid #9333ea!important;border-radius:16px!important;overflow:hidden!important;display:none!important;flex-direction:column!important;box-shadow:0 20px 60px rgba(147,51,234,.6)!important;pointer-events:auto!important;font-family:inherit!important;">
  <div style="background:linear-gradient(135deg,#9333ea,#ec4899);padding:.875rem 1rem;display:flex;align-items:center;justify-content:space-between;flex-shrink:0;">
    <div style="display:flex;align-items:center;gap:.625rem;">
      <div style="width:34px;height:34px;background:rgba(255,255,255,.2);border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:1.1rem;flex-shrink:0;">🤖</div>
      <div>
        <div style="color:#fff;font-weight:700;font-size:.9rem;line-height:1.2;">FTY Assistant</div>
        <div style="color:rgba(255,255,255,.8);font-size:.72rem;">🟢 En ligne – prêt à vous aider</div>
      </div>
    </div>
    <button onclick="ftyToggle()" style="background:rgba(255,255,255,.15);border:none;color:#fff;cursor:pointer;width:28px;height:28px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:1rem;flex-shrink:0;padding:0;">✕</button>
  </div>
  <div id="fty-msgs" style="flex:1;overflow-y:auto;padding:.875rem;display:flex;flex-direction:column;gap:.6rem;scrollbar-width:thin;scrollbar-color:#9333ea #0a0014;-webkit-overflow-scrolling:touch;">
    <div class="fty-bot-msg" style="background:rgba(147,51,234,.25);border:1px solid rgba(147,51,234,.35);border-radius:0 10px 10px 10px;padding:.65rem .875rem;color:#fff;font-size:.85rem;max-width:92%;line-height:1.45;">
      👋 Bonjour ! Je suis l'assistant FTY.<br>
      <span style="color:rgba(255,255,255,.65);font-size:.78rem;">Demandez-moi : <em>rejoindre, matchs, tactique, règles, recrutement, discord…</em></span>
    </div>
  </div>
  <div style="padding:.75rem;border-top:1px solid rgba(147,51,234,.3);display:flex;gap:.5rem;flex-shrink:0;background:rgba(5,0,12,.8);">
    <input id="fty-inp" type="text" placeholder="Votre message…"
      style="flex:1;background:rgba(147,51,234,.12);border:1px solid rgba(147,51,234,.4);border-radius:8px;padding:.6rem .875rem;color:#fff;font-size:.875rem;outline:none;min-width:0;font-family:inherit;"
      onkeydown="if(event.key==='Enter'&&!event.shiftKey){event.preventDefault();ftySend();}"
    >
    <button onclick="ftySend()" style="background:linear-gradient(135deg,#9333ea,#ec4899);border:none;border-radius:8px;width:38px;height:38px;color:#fff;cursor:pointer;font-size:.95rem;display:flex;align-items:center;justify-content:center;flex-shrink:0;padding:0;">➤</button>
  </div>
</div>
...
.fty-bot-msg { background:rgba(147,51,234,.22);border:1px solid rgba(147,51,234,.32);border-radius:0 10px 10px 10px;padding:.65rem .875rem;color:#fff;font-size:.85rem;max-width:92%;line-height:1.45;word-break:break-word; }
.fty-user-msg { background:linear-gradient(135deg,rgba(147,51,234,.55),rgba(236,72,153,.45));border-radius:10px 10px 0 10px;padding:.65rem .875rem;color:#fff;font-size:.85rem;max-width:85%;align-self:flex-end;word-break:break-word; }
#fty-msgs::-webkit-scrollbar { width:4px; }
#fty-msgs::-webkit-scrollbar-track { background:#0a0014; }
#fty-msgs::-webkit-scrollbar-thumb { background:#9333ea;border-radius:2px; }
@media(max-width:480px){
  #fty-chat-win{width:calc(100vw - 16px)!important;height:calc(100vh - 100px)!important;right:8px!important;bottom:80px!important;}
...
(function(){
  ...
  function ftyAddMsg(text, type) {
    var msgs = document.getElementById('fty-msgs');
    if (!msgs) return null;
    var d = document.createElement('div');
  ...
  window.ftyToggle = function() {
    var win = document.getElementById('fty-chat-win');
    if (!win) return;
    var open = win.classList.contains('fty-open');
    if (open) {
      win.classList.remove('fty-open');
      win.style.display = 'none';
    } else {
      win.classList.add('fty-open');
  ...
  window.ftySend = async function() {
    ...
    var typing = ftyAddMsg('<span style="opacity:.6">✏️ En train d'écrire…</span>', 'bot');
    ...
    try {
      ...
      ftyAddMsg(data.reply || '🤖 Pas de réponse reçue.', 'bot');
    } catch(err) {
      if (typing) typing.remove();
      ftyAddMsg('❌ Erreur de connexion au serveur. Réessayez.', 'bot');
    } finally {
      inp.disabled = false;
      inp.focus();
    }
...
<div id="fty-chat-win" style="position:fixed!important;bottom:94px!important;right:24px!important;z-index:2147483646!important;width:320px!important;height:440px!important;background:#0a0014!important;border:2px solid #9333ea!important;border-radius:16px!important;overflow:hidden!important;display:none!important;flex-direction:column!important;box-shadow:0 20px 60px rgba(147,51,234,.6)!important;pointer-events:auto!important;font-family:inherit!important;">
  ...
  <div id="fty-msgs" style="flex:1;overflow-y:auto;padding:.875rem;display:flex;flex-direction:column;gap:.6rem;scrollbar-width:thin;scrollbar-color:#9333ea #0a0014;-webkit-overflow-scrolling:touch;">
    <div style="background:rgba(147,51,234,.22);border:1px solid rgba(147,51,234,.32);border-radius:0 10px 10px 10px;padding:.65rem .875rem;color:#fff;font-size:.85rem;max-width:92%;line-height:1.45;">
      👋 Bonjour ! Je suis l'assistant FTY.<br>
      <span style="color:rgba(255,255,255,.65);font-size:.78rem;">Essayez : rejoindre, matchs, tactique, règles, recrutement, discord…</span>
    </div>
  </div>
  <div style="padding:.75rem;border-top:1px solid rgba(147,51,234,.3);display:flex;gap:.5rem;flex-shrink:0;background:rgba(5,0,12,.8);">
    <input id="fty-inp" type="text" placeholder="Votre message…"
...
(function(){
  ...
  window.ftyToggle=function(){
    ...
    if(!w)return;
    var open=w.classList.contains("fty-open");
    if(open){w.classList.remove("fty-open");w.style.setProperty("display","none","important");}
    else{w.classList.add("fty-open");w.style.setProperty("display","flex","important");setTimeout(function(){var i=document.getElementById("fty-inp");if(i)i.focus();},80);}
  };
  window.ftySend=async function(){
    var inp=document.getElementById("fty-inp");
    if(!inp)return;
    ...
    ftyAddMsg(msg,"user");
    var t=ftyAddMsg("<span style=\"opacity:.6\">✏️ En train d\'écrire…</span>","bot");
    try{
      var r=await fetch("/api/chatbot",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({message:msg})});
      if(t)t.remove();
      if(!r.ok)throw new Error("HTTP "+r.status);
      var data=await r.json();
      ftyAddMsg(data.reply||"🤖 Aucune réponse reçue.","bot");
    }catch(e){
      if(t)t.remove();
      ftyAddMsg("❌ Erreur de connexion. Réessayez.","bot");
    }finally{
      inp.disabled=false;
      inp.focus();
    }
