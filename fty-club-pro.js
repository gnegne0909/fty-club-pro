...
function initDB() {
    ...
    // ---- Migration automatique (DB existante) ----
    const dbM = readDB();
    let changed = false;
    dbM.users.forEach(u => {
    ...
    if (!dbM.notifications) { dbM.notifications = []; changed = true; }
    if (!dbM.messages) { dbM.messages = []; changed = true; }
    if (!dbM.notes) { dbM.notes = []; changed = true; }
    if (!dbM.blockedIPs) { dbM.blockedIPs = []; changed = true; }
    if (changed) writeDB(dbM);
}
