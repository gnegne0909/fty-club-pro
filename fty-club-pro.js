...
const SUPER_ADMIN_DISCORD_ID = '762763743476105286';
...
<script>
// ====== PROTECTION ANTI-F12 & ANTI-SCREENSHOT PANEL ======
(function(){
    var w = 0, max = 3;
    ...
    function warn(t) {
        ...
        n.style.cssText = 'position:fixed;top:20px;right:20px;background:linear-gradient(135deg,#ef4444,#dc2626);color:#fff;padding:1rem 1.5rem;border-radius:12px;box-shadow:0 10px 40px rgba(239,68,68,0.4);z-index:999999;font-weight:700;font-size:.95rem;';
        n.innerHTML = '<div style="display:flex;align-items:center;gap:.75rem;"><span style="font-size:1.5rem;">🚫</span><div><div>' + t + '</div><div style="font-size:.75rem;opacity:.9;margin-top:.25rem;">Avertissement ' + w + '/' + max + '</div></div></div>';
        document.body.appendChild(n);
        setTimeout(function(){ n.remove(); }, 3000);
        if (w >= max) {
            document.body.style.filter = 'blur(30px)';
            var l = document.createElement('div');
            l.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.98);display:flex;align-items:center;justify-content:center;z-index:99999999;color:white;font-size:1.5rem;font-weight:700;text-align:center;flex-direction:column;gap:1.5rem;';
            l.innerHTML = '<div style="font-size:6rem;">🔒</div><div>Accès bloqué</div><div style="font-size:1rem;opacity:.7;">Trop de tentatives</div>';
            document.body.appendChild(l);
            setTimeout(function(){ location.href = '/'; }, 3000);
        }
    }
    
    // Anti-DevTools (détection par taille fenêtre)
    var devOpen = false;
    setInterval(function() {
        ...
        if (window.outerWidth - window.innerWidth > th || window.outerHeight - window.innerHeight > th) {
            if (!devOpen) {
                ...
                d.id = 'devtools-overlay';
                d.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.95);display:flex;align-items:center;justify-content:center;z-index:9999999;color:white;font-size:2rem;font-weight:900;text-align:center;flex-direction:column;gap:1rem;';
                d.innerHTML = '<div style="font-size:5rem;">🚫</div><div>DevTools détectés</div><div style="font-size:1rem;opacity:.7;">Fermez les DevTools pour continuer</div>';
    ...
    // Blocage F12 / Ctrl+Shift+I / Ctrl+U
    document.addEventListener('keydown', function(e) {
        if (e.key === 'F12' || e.keyCode === 123 ||
            ...
            e.preventDefault();
            warn('Action bloquée sur cette page');
            return false;
        }
    });
    ...
    // Watermark avec IP visible via JS (côté client)
    var wm = document.createElement('div');
    wm.style.cssText = 'position:fixed;top:50%;left:50%;transform:translate(-50%,-50%) rotate(-45deg);font-size:5rem;color:rgba(147,51,234,.04);pointer-events:none;z-index:999997;user-select:none;font-weight:900;white-space:nowrap;';
...
<!-- ═══════════ CHATBOT FTY - HTML ═══════════ -->
<button id="fty-chat-btn" onclick="ftyToggle()" title="Chat FTY">💬</button>
<div id="fty-chat-win">
  <div style="background:linear-gradient(135deg,#9333ea,#ec4899);padding:.875rem 1rem;display:flex;align-items:center;justify-content:space-between;flex-shrink:0;">
    <div style="display:flex;align-items:center;gap:.625rem;">
      <div style="width:32px;height:32px;background:rgba(255,255,255,.2);border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:1rem;">🤖</div>
      <div><div style="color:#fff;font-weight:700;font-size:.9rem;">FTY Assistant</div><div style="color:rgba(255,255,255,.75);font-size:.75rem;">🟢 En ligne</div></div>
    </div>
    <button onclick="ftyToggle()" style="background:none;border:none;color:#fff;cursor:pointer;font-size:1.25rem;width:32px;height:32px;border-radius:50%;display:flex;align-items:center;justify-content:center;">✕</button>
  </div>
  <div id="fty-msgs">
    <div class="msg-bot">👋 Bonjour ! Je suis l'assistant FTY. Comment puis-je vous aider ?<br><small style="opacity:.7;margin-top:.25rem;display:block;">Essayez : rejoindre, matchs, tactique, recrutement</small></div>
  </div>
  <div style="padding:.75rem;border-top:1px solid rgba(147,51,234,.3);display:flex;gap:.5rem;flex-shrink:0;">
    <input id="fty-inp" type="text" placeholder="Votre message..." 
      style="flex:1;background:rgba(147,51,234,.15);border:1px solid rgba(147,51,234,.4);border-radius:8px;padding:.625rem .875rem;color:#fff;font-size:.875rem;outline:none;"
      onkeydown="if(event.key==='Enter')ftySend()"
    >
    <button onclick="ftySend()" style="background:linear-gradient(135deg,#9333ea,#ec4899);border:none;border-radius:8px;width:40px;height:40px;color:#fff;cursor:pointer;font-size:1rem;display:flex;align-items:center;justify-content:center;flex-shrink:0;">➤</button>
  </div>
</div>
<script>
...
window.addEventListener('DOMContentLoaded', function() {
    ...
    const btn = document.getElementById('fty-chat-btn');
...
async function ftySend() {
    ...
    var t = ftyAddMsg('✏️ En train d\'écrire...', 'bot');
...
(function(){
    const p=[/\\/panel\\/owner/,/\\/panel\\/settings/,/\\/panel\\/users/,/\\/panel\\/security/,/\\/panel\\/logs/];
    ...
    function d(){
        const th=160;
        const wt=window.outerWidth-window.innerWidth>th;
        const ht=window.outerHeight-window.innerHeight>th;
        if(wt||ht){
            document.body.style.filter='blur(20px)';
...
<!-- ═══════════ CHATBOT FTY PANEL ═══════════ -->
<button id="fty-chat-btn" onclick="ftyToggle()" title="Chat FTY" style="position:fixed!important;bottom:24px!important;right:24px!important;z-index:999999!important;width:58px!important;height:58px!important;border-radius:50%!important;background:linear-gradient(135deg,#9333ea,#ec4899)!important;border:none!important;cursor:pointer!important;font-size:1.5rem!important;box-shadow:0 6px 24px rgba(147,51,234,0.6)!important;display:flex!important;align-items:center!important;justify-content:center!important;">💬</button>
<div id="fty-chat-win" style="position:fixed!important;bottom:94px!important;right:24px!important;z-index:999998!important;width:320px!important;height:420px!important;background:#0a0014!important;border:2px solid #9333ea!important;border-radius:16px!important;overflow:hidden!important;display:none!important;flex-direction:column!important;box-shadow:0 20px 60px rgba(147,51,234,0.5)!important;">
  <div style="background:linear-gradient(135deg,#9333ea,#ec4899);padding:.875rem 1rem;display:flex;align-items:center;justify-content:space-between;flex-shrink:0;">
    <div style="display:flex;align-items:center;gap:.625rem;">
      <div style="width:32px;height:32px;background:rgba(255,255,255,.2);border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:1rem;">🤖</div>
      <div><div style="color:#fff;font-weight:700;font-size:.9rem;">FTY Assistant</div><div style="color:rgba(255,255,255,.75);font-size:.75rem;">🟢 En ligne</div></div>
  ...
  <div id="fty-msgs" style="flex:1;overflow-y:auto;padding:1rem;display:flex;flex-direction:column;gap:0.6rem;scrollbar-width:thin;scrollbar-color:#9333ea #0a0014;">
    <div style="background:rgba(147,51,234,0.2);border-radius:8px 8px 8px 0;padding:0.65rem 0.875rem;color:#fff;font-size:0.875rem;max-width:88%;border:1px solid rgba(147,51,234,0.3);">👋 Bonjour ! Je suis l'assistant FTY. Comment puis-je vous aider ?</div>
  </div>
  <div style="padding:.75rem;border-top:1px solid rgba(147,51,234,.3);display:flex;gap:.5rem;flex-shrink:0;">
    <input id="fty-inp" type="text" placeholder="Votre message..." 
      style="flex:1;background:rgba(147,51,234,.15);border:1px solid rgba(147,51,234,.4);border-radius:8px;padding:.625rem .875rem;color:#fff;font-size:.875rem;outline:none;"
...
<style>
#fty-chat-win.open { display:flex!important; animation:chatIn .2s ease; }
@keyframes chatIn { from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:translateY(0)} }
.fty-msg-bot{background:rgba(147,51,234,0.2);border-radius:8px 8px 8px 0;padding:.65rem .875rem;color:#fff;font-size:.875rem;max-width:88%;border:1px solid rgba(147,51,234,.3);}
.fty-msg-user{background:linear-gradient(135deg,rgba(147,51,234,.5),rgba(236,72,153,.4));border-radius:8px 8px 0 8px;padding:.65rem .875rem;color:#fff;font-size:.875rem;max-width:88%;align-self:flex-end;}
...
function ftyToggle() {
    ...
    if (w.classList.contains('open')) {
        ...
        setTimeout(function(){ var i=document.getElementById('fty-inp'); if(i) i.focus(); }, 50);
    }
}
...
function ftyAddMsg(txt, type) {
    ...
    if (!m) return null;
    var d = document.createElement('div');
    d.className = type === 'user' ? (d.className = 'msg-user fty-msg-user') : 'msg-bot fty-msg-bot';
    d.style.cssText = type === 'user' 
        ? 'background:linear-gradient(135deg,rgba(147,51,234,.5),rgba(236,72,153,.4));border-radius:8px 8px 0 8px;padding:.65rem .875rem;color:#fff;font-size:.875rem;max-width:88%;align-self:flex-end;'
        : 'background:rgba(147,51,234,0.2);border-radius:8px 8px 8px 0;padding:.65rem .875rem;color:#fff;font-size:.875rem;max-width:88%;border:1px solid rgba(147,51,234,.3);';
    d.innerHTML = txt;
    m.appendChild(d);
    m.scrollTop = m.scrollHeight;
    return d;
...
async function ftySend() {
    ...
    try {
        ...
        var d = await r.json();
        ftyAddMsg(d.reply || '🤖 Pas de réponse', 'bot');
    } catch(e) {
        if (t) t.remove();
