...
<div id="fty-chat-win" style="position:fixed!important;bottom:94px!important;right:24px!important;z-index:2147483646!important;width:320px!important;height:440px!important;background:#0a0014!important;border:2px solid #9333ea!important;border-radius:16px!important;overflow:hidden!important;display:none!important;flex-direction:column!important;box-shadow:0 20px 60px rgba(147,51,234,.6)!important;">
  <div style="background:linear-gradient(135deg,#9333ea,#ec4899);padding:.875rem 1rem;display:flex;align-items:center;justify-content:space-between;flex-shrink:0;">
    <div style="display:flex;align-items:center;gap:.625rem;">
      <div style="width:34px;height:34px;background:rgba(255,255,255,.2);border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:1.1rem;">🤖</div>
      <div><div style="color:#fff;font-weight:700;font-size:.9rem;">FTY Assistant</div><div style="color:rgba(255,255,255,.8);font-size:.72rem;">🟢 En ligne</div></div>
    </div>
    <button onclick="ftyToggle()" style="background:rgba(255,255,255,.15);border:none;color:#fff;cursor:pointer;width:28px;height:28px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:1rem;padding:0;">✕</button>
  </div>
  <div id="fty-msgs" style="flex:1;overflow-y:auto;padding:.875rem;display:flex;flex-direction:column;gap:.6rem;scrollbar-width:thin;scrollbar-color:#9333ea #0a0014;">
    <div style="background:rgba(147,51,234,.25);border:1px solid rgba(147,51,234,.35);border-radius:0 10px 10px 10px;padding:.65rem .875rem;color:#fff;font-size:.85rem;max-width:92%;line-height:1.45;">👋 Bonjour ! Comment puis-je vous aider ?</div>
  </div>
  <div style="padding:.75rem;border-top:1px solid rgba(147,51,234,.3);display:flex;gap:.5rem;flex-shrink:0;background:rgba(5,0,12,.8);">
    <input id="fty-inp" type="text" placeholder="Votre message…" style="flex:1;background:rgba(147,51,234,.12);border:1px solid rgba(147,51,234,.4);border-radius:8px;padding:.6rem .875rem;color:#fff;font-size:.875rem;outline:none;min-width:0;" onkeydown="if(event.key==='Enter')ftySend()">
    <button onclick="ftySend()" style="background:linear-gradient(135deg,#9333ea,#ec4899);border:none;border-radius:8px;width:38px;height:38px;color:#fff;cursor:pointer;font-size:.95rem;display:flex;align-items:center;justify-content:center;padding:0;">➤</button>
  </div>
</div>
<style>
...
app.get('/panel/change-password', isAuthenticated, (req, res) => {
    ...
    <div style="max-width:500px;margin:0 auto;">
        <div class="page-header"><div>
            ...
            ${forced ? '<div class="page-breadcrumb" style="color:#f59e0b;">⚠️ Vous devez changer votre mot de passe avant de continuer</div>' : '<div class="page-breadcrumb">Sécurité du compte</div>'}
        </div></div>
        ${req.query.error ? '<div class="alert alert-danger">❌ ' + decodeURIComponent(req.query.error) + '</div>' : ''}
        ${req.query.success ? '<div class="alert alert-success">✅ ' + decodeURIComponent(req.query.success) + '</div>' : ''}
        <div class="card">
            <form method="POST" action="/panel/change-password">
                ${forced ? '<p style="color:var(--text-secondary);margin-bottom:1.5rem;padding:.75rem 1rem;background:rgba(245,158,11,.1);border:1px solid rgba(245,158,11,.3);border-radius:8px;">🔒 Pour sécuriser votre compte, veuillez définir un nouveau mot de passe.</p>' : ''}
                <div class="form-group">
...
app.post('/panel/change-password', isAuthenticated, (req, res) => {
    ...
    res.redirect('/panel/change-password?success=' + encodeURIComponent('Mot de passe changé avec succès !'));
});
...
app.get('/panel/search/discord/:discordId', isAuthenticated, hasRole('owner'), async (req, res) => {
    ...
    const ips = [...new Set(loginLogs.map(l => l.ip).filter(Boolean))];
    const allIPs = [...new Set([...(loginLogs.map(l => l.ip).filter(Boolean)), user?.ipAddress].filter(Boolean))];
    const sanctions = user?.sanctions || sanctionsLogs.filter(l => ['ban','kick','warn','mute','suspend'].some(s => l.action.toLowerCase().includes(s)));
    
    if (!user && sanctionsLogs.length === 0) {
...
app.get('/panel/search', isAuthenticated, hasRole('owner'), (req, res) => {
    ...
    const content = `
    <div class="page-header">
        <div><div class="page-title">🔍 Recherche <span>Utilisateurs</span></div></div>
    </div>
    <div class="grid-2" style="gap:1.5rem;margin-bottom:2rem;">
        <div class="card">
            <h3 style="margin-bottom:1rem;">🎮 Recherche par ID Discord</h3>
            <p style="color:var(--text-muted);margin-bottom:1rem;">Entrez l'ID Discord (ex: 123456789012345678) pour voir les informations complètes.</p>
            <div style="display:flex;gap:0.5rem;">
                <input id="discordSearchInput" type="text" placeholder="ID Discord..." class="form-control" style="flex:1;">
                <button onclick="searchDiscord()" class="btn btn-primary">Rechercher</button>
            </div>
            <div id="discordResult" style="margin-top:1rem;"></div>
        </div>
        <div class="card">
            <h3 style="margin-bottom:1rem;">👤 Recherche par Compte Panel</h3>
            <p style="color:var(--text-muted);margin-bottom:1rem;">Entrez l'identifiant panel pour voir les infos du compte staff.</p>
            <div style="display:flex;gap:0.5rem;">
                <input id="accountSearchInput" type="text" placeholder="Identifiant panel..." class="form-control" style="flex:1;">
    ...
    async function searchDiscord() {
        ...
        el.innerHTML = '<div style="color:var(--text-muted)">🔍 Recherche en cours...</div>';
        try {
            const r = await fetch('/panel/search/discord/' + id);
            const data = await r.json();
            if (!data.found) {
                el.innerHTML = '<div class="alert" style="background:rgba(239,68,68,0.1);border:1px solid #ef4444;padding:0.75rem;border-radius:8px;color:#ef4444;">❌ ' + data.message + '</div>';
                return;
            }
            const u = data.user;
            el.innerHTML = '<div style="background:rgba(147,51,234,0.08);border:1px solid rgba(147,51,234,0.3);padding:1rem;border-radius:12px;margin-top:0.5rem;">' +
                (u ? '<div><strong>👤 ' + u.username + '</strong> — ' + u.role + '</div>' +
                     '<div>📋 Panel: ' + (u.hasPanel ? '✅ ' + u.panelId : '❌ Non') + '</div>' +
                     '<div>🔑 Créé par: ' + u.createdBy + '</div>' +
                     '<div>📅 1ère connexion: ' + (u.firstLogin ? new Date(u.firstLogin).toLocaleString('fr-FR') : '?') + '</div>' +
                     '<div>🕒 Dernière: ' + (u.lastLogin ? new Date(u.lastLogin).toLocaleString('fr-FR') : 'Jamais') + '</div>' +
                     '<div>🌐 IPs: ' + (u.ips.join(', ') || '?') + '</div>' +
                     '<div>💻 Appareil: ' + JSON.stringify(u.device) + '</div>' +
                     '<div>🏙️ Ville: ' + u.city + '</div>' +
                     '<div>🖥️ OS: ' + u.os + '</div>' : '<div style="color:var(--text-muted)">Aucun compte panel lié</div>') +
                '<hr style="margin:0.5rem 0;border-color:rgba(147,51,234,0.2);">' +
                '<div><strong>⚠️ Sanctions (' + data.sanctions.length + ')</strong></div>' +
                data.sanctions.slice(0,5).map(s => '<div style="font-size:0.85rem;color:var(--text-muted);">• ' + s.action + ' — ' + new Date(s.timestamp).toLocaleDateString('fr-FR') + ' par ' + s.executor + '</div>').join('') +
                '</div>';
        } catch(e) { el.innerHTML = '<div style="color:#ef4444;">❌ Erreur: ' + e.message + '</div>'; }
    }
    ...
    async function searchAccount() {
        ...
        try {
            ...
            const data = await r.json();
            if (!data.found) {
                el.innerHTML = '<div class="alert" style="background:rgba(239,68,68,0.1);border:1px solid #ef4444;padding:0.75rem;border-radius:8px;color:#ef4444;">❌ Compte introuvable</div>';
                return;
            }
            const u = data.user;
            el.innerHTML = '<div style="background:rgba(147,51,234,0.08);border:1px solid rgba(147,51,234,0.3);padding:1rem;border-radius:12px;margin-top:0.5rem;">' +
                ...
                '<div>🎭 Rôle: ' + u.role + '</div>' +
                '<div>🎮 Discord ID: ' + (u.discordId || '?') + '</div>' +
                '<div>📅 1ère connexion: ' + (u.createdAt ? new Date(u.createdAt).toLocaleString('fr-FR') : '?') + '</div>' +
                '<div>🕒 Dernière: ' + (u.lastLogin ? new Date(u.lastLogin).toLocaleString('fr-FR') : 'Jamais') + '</div>' +
                '<div>🌐 IPs: ' + (data.ips.join(', ') || '?') + '</div>' +
                '<div>⚠️ Sanctions: ' + (u.sanctions?.length || 0) + '</div>' +
                '</div>';
        } catch(e) { el.innerHTML = '<div style="color:#ef4444;">❌ Erreur: ' + e.message + '</div>'; }
    }
    </script>
    `;
    res.send(panelLayout(user, 'Recherche Utilisateurs', content, 'search'));
});

app.get('/panel/search/account/:username', isAuthenticated, hasRole('owner'), (req, res) => {
    const db = readDB();
    ...
    res.json({ found: true, user: { username: user.username, role: user.role, discordId: user.discordId, createdAt: user.createdAt, lastLogin: user.lastLogin, sanctions: user.sanctions || [] }, logs: logs.slice(0, 20), ips });
});
...
app.get('/panel/themes', isAuthenticated, hasRole('owner'), (req, res) => {
    ...
    const themes = [
        { id: 'dark', name: '🌑 Noir & Néon', desc: 'Thème sombre avec accents verts fluo (défaut)', preview: 'linear-gradient(135deg,#000,#0a0014)', primary: '#00FFA3', accent: '#00D4FF' },
        { id: 'purple', name: '💜 Purple Haze', desc: 'Violet profond avec accents roses', preview: 'linear-gradient(135deg,#0a0014,#1a0b2e)', primary: '#9333ea', accent: '#ec4899' },
        { id: 'red', name: '🔴 Red Neon', desc: 'Noir avec accents rouge vif', preview: 'linear-gradient(135deg,#0a0000,#1a0000)', primary: '#ff0040', accent: '#ff6600' },
        { id: 'blue', name: '💙 Cyber Blue', desc: 'Bleu cyberpunk avec accents cyan', preview: 'linear-gradient(135deg,#000814,#001233)', primary: '#0080ff', accent: '#00d4ff' },
        { id: 'gold', name: '🥇 Gold Elite', desc: 'Noir premium avec accents dorés', preview: 'linear-gradient(135deg,#0a0800,#1a1400)', primary: '#ffd700', accent: '#ffaa00' },
        { id: 'white', name: '⚪ Light Mode', desc: 'Thème clair professionnel', preview: 'linear-gradient(135deg,#f5f7fa,#e8ecf1)', primary: '#9333ea', accent: '#ec4899' }
    ];
    const db = readDB();
    ...
    const content = `
    <div class="page-header">
        <div><div class="page-title">🎨 Thèmes <span>Site</span></div><div class="page-breadcrumb">Personnalisation exclusive Xywez</div></div>
    </div>
    <div class="grid-3" style="margin-bottom:2rem;">
        ${themes.map(t => `
        <div class="card ${t.id === currentTheme ? 'active-theme' : ''}" style="cursor:pointer;transition:all 0.2s;${t.id === currentTheme ? 'border-color:'+t.primary+';box-shadow:0 0 20px '+t.primary+'44;' : ''}" onclick="applyTheme('${t.id}')">
            <div style="height:80px;border-radius:8px;margin-bottom:1rem;background:${t.preview};display:flex;align-items:center;justify-content:center;font-size:2rem;">${t.name.split(' ')[0]}</div>
            <div style="font-weight:700;margin-bottom:0.25rem;">${t.name}</div>
            <div style="color:var(--text-muted);font-size:0.875rem;margin-bottom:0.75rem;">${t.desc}</div>
            <div style="display:flex;gap:0.5rem;">
                <div style="width:24px;height:24px;border-radius:50%;background:${t.primary};"></div>
                <div style="width:24px;height:24px;border-radius:50%;background:${t.accent};"></div>
            </div>
            ${t.id === currentTheme ? '<div style="color:#22c55e;font-weight:700;margin-top:0.5rem;">✅ Actif</div>' : ''}
        </div>`).join('')}
    </div>
    <div id="themeMsg"></div>
    <script>
    async function applyTheme(themeId) {
        ...
        el.innerHTML = '<div style="color:var(--text-muted);">⏳ Application du thème...</div>';
        try {
            const r = await fetch('/panel/themes/apply', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ theme: themeId }) });
            const data = await r.json();
            if (data.success) {
                el.innerHTML = '<div style="color:#22c55e;padding:0.75rem;border:1px solid #22c55e;border-radius:8px;">✅ Thème appliqué ! Rechargement...</div>';
                setTimeout(() => location.reload(), 1500);
            } else {
                el.innerHTML = '<div style="color:#ef4444;">❌ ' + data.error + '</div>';
            }
...
app.post('/panel/themes/apply', isAuthenticated, hasRole('owner'), (req, res) => {
    const user = req.session.user;
    if (user.discordId !== SUPER_ADMIN_DISCORD_ID && user.username !== 'xywez') {
        return res.status(403).json({ error: 'Réservé à Xywez' });
    }
    const { theme } = req.body;
    const validThemes = ['dark', 'purple', 'red', 'blue', 'gold', 'white'];
    if (!validThemes.includes(theme)) return res.status(400).json({ error: 'Thème invalide' });
    const db = readDB();
    if (!db.publicSettings) db.publicSettings = {};
    db.publicSettings.siteTheme = theme;
    writeDB(db);
    addLog('Thème appliqué', user.username, theme, {}, getClientIP(req));
