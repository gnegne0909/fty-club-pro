...
function checkMustChangePassword(req, res, next) {
    ...
    // Avec app.use('/panel/*', ...), req.path contient le chemin COMPLET (ex: /panel/change-password)
    const path = req.path.split('?')[0];
    if (path === '/panel/change-password' || path === '/panel/change-password/' ||
        path === '/change-password' || path === '/change-password/' ||
        path === '/panel/logout' || path === '/logout') return next();
