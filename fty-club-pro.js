...
// Les middlewares rateLimiter et checkBlockedIP sont enregistrés après leur définition (voir plus bas)

// Sur Render le dossier src est read-only, on stocke dans /tmp qui persiste pendant la session
...
// Valeurs par défaut de la DB si un champ manque
const DB_DEFAULTS = {
    users: [], applications: [], notifications: [], teamMembers: [],
    matches: [], announcements: [], logs: [], notes: [], blockedIPs: [],
    antiRaid: { enabled: true, joinThreshold: 5, timeWindow: 10, doubleCompteEnabled: true, antiLinkEnabled: true },
    dmTickets: [], messages: [],
    stats: { wins: 15, draws: 4, losses: 2, goals: 58, goalsAgainst: 21, winRate: 71.4 },
...
function readDB() {
    try {
        ...
        let changed = false;
        for (const [key, def] of Object.entries(DB_DEFAULTS)) {
            if (data[key] === undefined || data[key] === null) {
                data[key] = def;
                changed = true;
            }
        }
        // Garantir stats en détail
        if (!data.stats || typeof data.stats !== 'object') {
            data.stats = DB_DEFAULTS.stats;
            changed = true;
        }
        if (data.stats.wins === undefined) { data.stats.wins = 0; changed = true; }
        if (data.stats.draws === undefined) { data.stats.draws = 0; changed = true; }
        if (data.stats.losses === undefined) { data.stats.losses = 0; changed = true; }
        if (data.stats.goals === undefined) { data.stats.goals = 0; changed = true; }
        if (data.stats.goalsAgainst === undefined) { data.stats.goalsAgainst = 0; changed = true; }
        if (data.stats.winRate === undefined) { data.stats.winRate = 0; changed = true; }
        if (changed) fs.writeFileSync(DB_FILE, JSON.stringify(data, null, 2));
        return data;
    } catch (e) {
        console.error('❌ Erreur lecture DB, réinitialisation:', e.message);
        initDB();
...
// ✅ Enregistrement des middlewares de sécurité (après leur définition)
app.use(rateLimiter);
