...
// ── Accueil ──────────────────────────────────────────────────────────
// ============================================================
// TRACKER VISITEURS PUBLICS — GeoIP automatique
// /, /candidature, /boutique, /guide, /palmares, /recrutement, /contact, /tactique
// ============================================================
const PUBLIC_TRACKED = ['/', '/candidature', '/boutique', '/contact', '/guide', '/palmares', '/recrutement', '/tactique'];
app.use(async (req, res, next) => {
    if (req.method !== 'GET') return next();
    if (!PUBLIC_TRACKED.includes(req.path)) return next();
    const ip = getClientIP(req);
    if (!ip || ip === '127.0.0.1' || ip === 'unknown' || ip === '[PROTECTED]' || ip.startsWith('192.168') || ip.startsWith('10.')) return next();
    setImmediate(async () => {
        try {
            const geo = await getGeoIP(ip);
            const ci  = getClientInfo(req);
            const db2 = readDB();
            if (!db2.publicVisitors) db2.publicVisitors = [];
            db2.publicVisitors.unshift({
                id: 'vis_' + Date.now(),
                path: req.path, ip,
                device: ci.device||'', browser: ci.browser||'', os: ci.os||'',
                ua: req.headers['user-agent']||'',
                referer: req.headers['referer']||'',
                geo: geo||null,
                ts: new Date().toISOString(),
            });
            if (db2.publicVisitors.length > 1000) db2.publicVisitors = db2.publicVisitors.slice(0, 1000);
            writeDB(db2);
        } catch(e) {}
    });
    next();
...
<script>
// ====== PROTECTION ANTI-F12 ULTRA V4 & ANTI-SCREEN & MOBILE ======
(function(){
    var IS_XYWEZ=(window.__xywezBypass===true)||false;
    var w=0,max=3,blocked=false,devOpen=false;

    // CSS
    var css=document.createElement('style');
    css.textContent=[
        '@keyframes ftySlideIn{from{opacity:0;transform:translateX(120%)}to{opacity:1;transform:translateX(0)}}',
        '@keyframes ftyPulse{0%,100%{opacity:1}50%{opacity:.4}}',
        '#fty-devtools-overlay{position:fixed;inset:0;background:rgba(0,0,0,.97);display:flex;align-items:center;justify-content:center;z-index:9999998;color:#fff;font-family:sans-serif;text-align:center;flex-direction:column;gap:1rem;backdrop-filter:blur(4px);}',
        '@media print{body *{visibility:hidden!important;}body::after{content:"\\1F512 FTY Club Pro - Impression interdite";visibility:visible!important;position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);font-size:2.5rem;font-weight:900;color:#9333ea;}}',
    ].join('');
    ...
    function showToast(t,col){
        ...
        n.style.cssText='position:fixed;top:16px;right:16px;background:linear-gradient(135deg,'+(col||'#ef4444')+',#991b1b);color:#fff;padding:.875rem 1.25rem;border-radius:12px;box-shadow:0 8px 30px rgba(239,68,68,.45);z-index:999999;font-weight:700;font-size:.88rem;animation:ftySlideIn .3s ease;pointer-events:none;max-width:290px;';
        n.innerHTML='<div style="display:flex;align-items:center;gap:.6rem;"><span style="font-size:1.3rem;">🚫</span><div><div>'+t+'</div><div style="font-size:.7rem;opacity:.8;margin-top:.15rem;">Avertissement '+w+'/'+max+' — FTY Security V4</div></div></div>';
    ...
    function hardBlock(){
        ...
        l.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.99);display:flex;align-items:center;justify-content:center;z-index:999999999;color:#fff;font-family:sans-serif;text-align:center;flex-direction:column;gap:1.5rem;padding:2rem;';
        l.innerHTML='<div style="font-size:5rem;animation:ftyPulse 1s infinite;">🔒</div><div style="font-size:1.6rem;font-weight:900;color:#ef4444;">SESSION BLOQUÉE</div><div style="font-size:.95rem;opacity:.7;max-width:360px;">Trop de violations détectées.<br>Redirection en cours...</div><div id="ftyCD" style="font-size:2rem;font-weight:900;color:#f59e0b;">5</div>';
        document.body.appendChild(l);
        var ct=5,iv=setInterval(function(){ct--;var el=document.getElementById('ftyCD');if(el)el.textContent=ct;if(ct<=0){clearInterval(iv);location.href='/panel/login';}},1000);
    }

    // Technique Netflix : layer video
    (function(){
        try{
            var cv=document.createElement('canvas');cv.width=1;cv.height=1;
            var ctx=cv.getContext('2d');ctx.fillStyle='black';ctx.fillRect(0,0,1,1);
            var vid=document.createElement('video');
            vid.srcObject=cv.captureStream(0);
            vid.style.cssText='position:fixed;inset:0;width:100%;height:100%;object-fit:cover;z-index:999996;pointer-events:none;opacity:0;';
            vid.disableRemotePlayback=true;vid.setAttribute('playsinline','');
            document.body.appendChild(vid);vid.play().catch(function(){});
        }catch(e){}
    })();

    // Logs serveur
    function logCapture(reason){
        try{fetch('/api/screenshot-attempt',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({reason:reason,ts:new Date().toISOString()})});}catch(e){}
    }
    function logEvent(reason,label){
        try{fetch('/api/security-event',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({reason:reason,label:label,ts:new Date().toISOString()})});}catch(e){}
    }
    ...
    if(!IS_XYWEZ){
        ...
        function checkDevBySize(){
            ...
            if(open&&!devOpen){
                ...
                var d=document.getElementById('fty-devtools-overlay')||document.createElement('div');
                d.id='fty-devtools-overlay';
                d.innerHTML='<div style="font-size:5rem;">🚫</div><div style="font-size:1.8rem;font-weight:900;">DevTools Détectés</div><div style="font-size:.95rem;opacity:.7;max-width:320px;margin-top:.5rem;">Fermez les outils de développement pour accéder au panel.</div><div style="margin-top:1rem;font-size:.8rem;opacity:.45;">FTY Security V4</div>';
                document.body.appendChild(d);
                logEvent('devtools:open','🔍 DevTools ouverts');
                if(devCC>=2)showToast('DevTools détectés plusieurs fois','#7c3aed');
            } else if(!open&&devOpen){
                devOpen=false;document.body.style.filter='';
                var o=document.getElementById('fty-devtools-overlay');if(o)o.remove();
            }
        }
        ...
        function debuggerTrap(){
            var s=performance.now();(function(){debugger;})();
            if(performance.now()-s>80&&!_dtC){
                _dtC=true;showToast('Debugger détecté','#dc2626');
                logEvent('debugger:detected','🐛 Debugger actif');
                setTimeout(function(){_dtC=false;},5000);
            }
        }
        setInterval(debuggerTrap,3000);

        // M3 : toString override
        var _dtO=false,_dtChk=/./;
        _dtChk.toString=function(){_dtO=true;return '';};
        setInterval(function(){
            _dtO=false;console.log('%c',_dtChk);
            if(_dtO&&!devOpen){showToast('Console ouverte','#dc2626');logEvent('console:open','💻 Console JS ouverte');}
        },2500);

        // M4 : Performance baseline
        var _perfB=null;
        setInterval(function(){
            var t0=performance.now();for(var i=0;i<1e5;i++){}var dt=performance.now()-t0;
            if(_perfB===null){_perfB=dt*3.5;return;}
            if(dt>_perfB&&!devOpen)showToast('Activité suspecte','#7c3aed');
        },8000);

        // M5 : Error.stack
        setInterval(function(){
            try{var e=new Error();if(e.stack&&e.stack.includes('debugger'))showToast('Debugger stack','#dc2626');}catch(ex){}
        },6000);

        // Clavier Desktop + Mobile
        document.addEventListener('keydown',function(e){
            ...
            if((e.ctrlKey||e.metaKey)&&!e.shiftKey&&['u','U','s','S','p','P'].includes(e.key)){e.preventDefault();showToast('Action bloquée');logCapture('keydown:Ctrl+'+e.key.toUpperCase());return false;}
            if(e.metaKey&&e.altKey&&['i','I'].includes(e.key)){e.preventDefault();showToast('DevTools macOS bloqué');return false;}
            if(e.key==='PrintScreen'||e.keyCode===44){
                e.preventDefault();
                try{navigator.clipboard.writeText('🔒 FTY Club Pro — Non autorisé');}catch(er){}
                showToast('Capture écran bloquée');logCapture('keydown:PrintScreen');
                document.body.style.filter='blur(25px)';
                setTimeout(function(){document.body.style.filter=devOpen?'blur(20px)':'';},2500);
                return false;
            }
            if(e.metaKey&&e.shiftKey&&['3','4','5'].includes(e.key)){
                logCapture('keydown:Mac-Screenshot-'+e.key);showToast('Screenshot macOS intercepté');
            }
        },true);
        document.addEventListener('keyup',function(e){
            if(e.key==='PrintScreen'||e.keyCode===44){
                try{navigator.clipboard.writeText('🔒 FTY Club Pro — Non autorisé');}catch(er){}
                showToast('Capture interceptée');logCapture('keyup:PrintScreen');
                document.body.style.filter='blur(25px)';
                setTimeout(function(){document.body.style.filter=devOpen?'blur(20px)':'';},2500);
            }
        },true);
        ...
        // Anti copier-coller
        function isField(el){return el&&(el.tagName==='INPUT'||el.tagName==='TEXTAREA'||el.tagName==='SELECT'||el.isContentEditable);}
        document.addEventListener('copy',function(e){
            if(!isField(document.activeElement)){e.preventDefault();if(e.clipboardData)e.clipboardData.setData('text/plain','🔒 Contenu protégé — FTY Club Pro');showToast('Copie désactivée','#7c3aed');}
        },true);
        document.addEventListener('cut',function(e){if(!isField(document.activeElement)){e.preventDefault();showToast('Couper désactivé','#7c3aed');}},true);
        document.addEventListener('keydown',function(e){
            if((e.ctrlKey||e.metaKey)&&['c','C','x','X','v','V','a','A'].includes(e.key)&&!isField(document.activeElement)){
                e.preventDefault();if(['c','C','x','X'].includes(e.key))showToast('Copie/Couper désactivé','#7c3aed');return false;
            }
        },true);

        // Clic droit + selection + drag
        document.addEventListener('contextmenu',function(e){e.preventDefault();showToast('Clic droit désactivé');return false;},true);
        document.addEventListener('selectstart',function(e){if(!isField(e.target)){e.preventDefault();return false;}},true);
        var bs=document.body.style;bs.userSelect=bs.webkitUserSelect=bs.MozUserSelect=bs.msUserSelect='none';
        document.addEventListener('dragstart',function(e){e.preventDefault();return false;},true);

        // Blocage partage ecran OBS/Discord/Zoom
        if(navigator.mediaDevices){
            navigator.mediaDevices.getDisplayMedia=function(){
                showToast('Partage d'écran bloqué','#dc2626');logCapture('getDisplayMedia:blocked');
                return Promise.reject(new DOMException('Désactivé — FTY Security V4','NotAllowedError'));
            };
            var _gum=navigator.mediaDevices.getUserMedia;
            navigator.mediaDevices.getUserMedia=function(con){
                if(con&&(con.video||con.screen)){showToast('Capture vidéo bloquée','#dc2626');logCapture('getUserMedia:blocked');return Promise.reject(new DOMException('Désactivé','NotAllowedError'));}
                return _gum?_gum.call(navigator.mediaDevices,con):Promise.reject(new Error('N/A'));
            };
        }

        // Anti-iframe
        if(window.self!==window.top){
            document.body.innerHTML='<div style="background:#000;color:#9333ea;display:flex;align-items:center;justify-content:center;height:100vh;font-size:1.5rem;font-weight:900;text-align:center;padding:2rem;">🔒 Accès interdit en iframe</div>';
            logEvent('iframe:detected','🖼 Embedding iFrame');hardBlock();
        }

        // Visibility : flou si onglet masque
        document.addEventListener('visibilitychange',function(){
            if(document.hidden){document.body.style.filter='blur(15px)';logEvent('visibility:hidden','👁 Onglet masqué');}
            else{setTimeout(function(){if(!devOpen)document.body.style.filter='';},400);}
        });

        // MOBILE : gestes screenshot (3 doigts)
        var _ts=0;
        document.addEventListener('touchstart',function(e){
            if(e.touches.length>=3)_ts=Date.now();
            if(e.target.tagName==='IMG')e.preventDefault();
        },{passive:false});
        document.addEventListener('touchend',function(e){
            if(_ts>0&&Date.now()-_ts<600&&e.touches.length===0){
                showToast('Geste screenshot détecté');logCapture('touch:3fingers');
                document.body.style.filter='blur(25px)';
                setTimeout(function(){document.body.style.filter=devOpen?'blur(20px)':'';},2000);
                _ts=0;
            }
        },true);

        // MOBILE : window blur (power+volume iOS / bouton screenshot Android)
        window.addEventListener('blur',function(){
            if(/iPhone|iPad|iPod|Android/i.test(navigator.userAgent)){
                document.body.style.filter='blur(25px)';logCapture('mobile:window-blur');
                setTimeout(function(){document.body.style.filter=devOpen?'blur(20px)':'';},3000);
            }
        });

        // Override console
        try{Object.defineProperty(console,'log',{get:function(){showToast('Console interceptée');return function(){};}});}catch(e){}
    }

    // Watermark (tout le monde)
    var _u=(window.__ftyUser||'PANEL'),_d=new Date().toLocaleString('fr-FR');
    function mkWM(txt,st){var d=document.createElement('div');d.style.cssText='pointer-events:none;z-index:999997;user-select:none;position:fixed;font-family:sans-serif;'+st;d.textContent=txt;document.body.appendChild(d);return d;}
    var wm =mkWM('FTY CLUB PRO — '+_u.toUpperCase(),'top:50%;left:50%;transform:translate(-50%,-50%) rotate(-35deg);font-size:4rem;color:rgba(147,51,234,.035);font-weight:900;white-space:nowrap;');
    var wm2=mkWM(_d,'top:70%;left:30%;transform:translate(-50%,-50%) rotate(-35deg);font-size:2rem;color:rgba(147,51,234,.025);font-weight:700;white-space:nowrap;');
    var wm3=mkWM('FTY-SEC-V4 • '+_u+' • '+_d,'bottom:8px;right:12px;font-size:.62rem;color:rgba(147,51,234,.12);font-family:monospace;letter-spacing:.04em;');
    setInterval(function(){var nd=new Date().toLocaleString('fr-FR');wm2.textContent=nd;wm3.textContent='FTY-SEC-V4 • '+_u+' • '+nd;},30000);
...
app.get('/panel/system', isAuthenticated, hasRole('owner'), (req, res) => {
    if (req.session.user.username !== 'xywez') {
        return res.status(403).send(errorPage('Acces Refuse', 'Reserve au compte owner xywez uniquement.'));
    }
    const db   = readDB();
    const user = req.session.user;
    const secEvents   = (db.securityEvents  || []).slice(0, 60);
    const pubVisitors = (db.publicVisitors   || []).slice(0, 60);

    function renderSecEvent(e) {
        const geo = e.geo || {};
        const flag  = geo.emoji || '🌍';
        const loc   = (geo.city && geo.country) ? geo.city+', '+geo.country : 'Inconnu';
        const isp   = geo.isp || '';
        const vpn   = geo.vpn   ? '<span style="color:#f59e0b;font-size:.7rem;">⚠️ VPN</span>'   : '';
        const proxy = geo.proxy ? '<span style="color:#ef4444;font-size:.7rem;">🔴 PROXY</span>' : '';
        const col   = e.type === 'capture' ? '#a855f7' : '#3b82f6';
        const dt    = new Date(e.ts).toLocaleString('fr-FR');
        return `<div style="background:rgba(10,10,20,.7);border:1px solid rgba(147,51,234,.15);border-left:3px solid ${col};border-radius:8px;padding:.75rem 1rem;margin-bottom:.5rem;">`
            +`<div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:.3rem;">`
            +`<div><span style="font-weight:700;font-size:.875rem;">${e.label}</span>`
            +` <span style="color:var(--text-muted);font-size:.75rem;">· ${e.username} <span style="color:#a855f7;">(${e.role})</span></span></div>`
            +`<span style="font-size:.7rem;color:var(--text-muted);">${dt}</span></div>`
            +`<div style="margin-top:.4rem;display:flex;flex-wrap:wrap;gap:.5rem;font-size:.75rem;color:var(--text-muted);align-items:center;">`
            +`<span>🖥 ${e.device||'?'}</span>`
            +`<span>🌐 ${e.browser||'?'}</span>`
            +`<span>💿 ${e.os||'?'}</span>`
            +`<span>🔌 <code style="background:rgba(255,255,255,.07);padding:.1rem .3rem;border-radius:4px;font-size:.7rem;">${e.ip||'?'}</code> ${vpn}${proxy}</span>`
            +`<span>${flag} ${loc}${isp ? ' · '+isp : ''}</span>`
            +`</div></div>`;
    }
    ...
    function renderVisitor(v) {
        ...
        const vpn   = geo.vpn   ? '<span style="color:#f59e0b;font-size:.7rem;">⚠️ VPN</span>'   : '';
        const proxy = geo.proxy ? '<span style="color:#ef4444;font-size:.7rem;">🔴 PROXY</span>' : '';
        const cols  = {'/'  :'#22c55e','/candidature':'#a855f7','/boutique':'#f59e0b','/guide':'#3b82f6',
                       '/contact':'#ec4899','/palmares':'#f97316','/recrutement':'#14b8a6','/tactique':'#8b5cf6'};
        const col   = cols[v.path] || '#6b7280';
        const dt    = new Date(v.ts).toLocaleString('fr-FR');
        return `<div style="background:rgba(10,10,20,.7);border:1px solid rgba(147,51,234,.15);border-left:3px solid ${col};border-radius:8px;padding:.75rem 1rem;margin-bottom:.5rem;">`
            +`<div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:.3rem;">`
            +`<span style="color:${col};font-weight:700;font-size:.875rem;">${v.path}</span>`
            +`<span style="font-size:.7rem;color:var(--text-muted);">${dt}</span></div>`
            +`<div style="margin-top:.4rem;display:flex;flex-wrap:wrap;gap:.5rem;font-size:.75rem;color:var(--text-muted);align-items:center;">`
            +`<span>🖥 ${v.device||'?'}</span>`
            +`<span>🌐 ${v.browser||'?'}</span>`
            +`<span>💿 ${v.os||'?'}</span>`
            +`<span>🔌 <code style="background:rgba(255,255,255,.07);padding:.1rem .3rem;border-radius:4px;font-size:.7rem;">${v.ip||'?'}</code> ${vpn}${proxy}</span>`
            +`<span>${flag} ${loc}${isp ? ' · '+isp : ''}</span>`
            +(v.referer ? `<span>↩ ${v.referer.slice(0,50)}</span>` : '')
            +`</div></div>`;
    }
    ...
    <div class="alert alert-warning">⚠️ Les actions ici sont irréversibles. Procédez avec prudence.</div>

    <div class="grid-2">
        <div class="card"><div class="card-header"><h2 class="card-title">👑 Gestion Owners</h2></div>
            <p class="text-secondary mb-16">Gérer les comptes avec privilèges Owner</p>
            <a href="/panel/system/owners" class="btn btn-primary btn-full">👑 Gérer les Owners</a>
        </div>
        <div class="card"><div class="card-header"><h2 class="card-title">💾 Base de Données</h2></div>
            <p class="text-secondary mb-16">Taille: ${JSON.stringify(db).length} caractères<br>Users: ${db.users.length} · Logs: ${(db.logs||[]).length}</p>
        ...
        <div class="card"><div class="card-header"><h2 class="card-title">⚙️ Serveur</h2></div>
            <div style="font-size:.9rem;">
                <div style="padding:.75rem 0;border-bottom:1px solid var(--border);">Node.js: <strong>${process.version}</strong></div>
                <div style="padding:.75rem 0;border-bottom:1px solid var(--border);">Port: <strong>${PORT}</strong></div>
                <div style="padding:.75rem 0;border-bottom:1px solid var(--border);">Uptime: <strong>${Math.floor(process.uptime()/60)} min</strong></div>
                <div style="padding:.75rem 0;">Mémoire: <strong>${Math.round(process.memoryUsage().heapUsed/1024/1024)} MB</strong></div>
            </div>
        </div>
        <div class="card"><div class="card-header"><h2 class="card-title">📡 Stats Sécurité</h2></div>
            <div style="font-size:.875rem;">
                <div style="padding:.6rem 0;border-bottom:1px solid var(--border);">📸 Captures: <strong style="color:#ef4444;">${(db.securityEvents||[]).filter(e=>e.type==='capture').length}</strong></div>
                <div style="padding:.6rem 0;border-bottom:1px solid var(--border);">🛡️ Evénements sécurité: <strong style="color:#f59e0b;">${(db.securityEvents||[]).filter(e=>e.type==='security').length}</strong></div>
                <div style="padding:.6rem 0;border-bottom:1px solid var(--border);">📹 Partages écran bloqués: <strong style="color:#a855f7;">${(db.securityEvents||[]).filter(e=>e.rawReason==='getDisplayMedia:blocked').length}</strong></div>
                <div style="padding:.6rem 0;border-bottom:1px solid var(--border);">📱 Tentatives mobile: <strong style="color:#3b82f6;">${(db.securityEvents||[]).filter(e=>e.rawReason&&(e.rawReason.startsWith('mobile:')||e.rawReason.startsWith('touch:'))).length}</strong></div>
                <div style="padding:.6rem 0;">👁 Visiteurs publics: <strong style="color:#22c55e;">${(db.publicVisitors||[]).length}</strong></div>
            </div>
        </div>
    </div>
    ...
    <div style="margin-top:2rem;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem;flex-wrap:wrap;gap:.5rem;">
            <div>
                <div class="page-title" style="font-size:1.3rem;">📡 Détection <span>Captures &amp; Streams</span></div>
                <div class="page-breadcrumb">Screenshots · PrintScreen · OBS · Discord · DevTools · Mobile · avec IP et GeoIP</div>
            </div>
            <button onclick="clearSec()" style="background:rgba(239,68,68,.15);border:1px solid rgba(239,68,68,.3);color:#ef4444;padding:.5rem 1rem;border-radius:8px;cursor:pointer;font-size:.8rem;font-weight:700;">🗑️ Effacer</button>
        </div>
        ${secEvents.length === 0
            ? '<div class="card" style="text-align:center;padding:2rem;color:var(--text-muted);">✅ Aucun événement de sécurité</div>'
            : secEvents.map(renderSecEvent).join('')}
        ${(db.securityEvents||[]).length > 60 ? '<div style="text-align:center;color:var(--text-muted);font-size:.8rem;margin-top:.5rem;">... et '+((db.securityEvents||[]).length-60)+' autres événements</div>' : ''}
    </div>

    <!-- VISITEURS PUBLICS GEOIP -->
    <div style="margin-top:2rem;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem;flex-wrap:wrap;gap:.5rem;">
            <div>
                <div class="page-title" style="font-size:1.3rem;">🌍 Visiteurs <span>Site Public</span></div>
                <div class="page-breadcrumb">Accueil · Candidature · Boutique · Guide · Palmarès · Contact — IP + GeoIP en temps réel</div>
            </div>
            <div style="display:flex;gap:.5rem;flex-wrap:wrap;">
                <select id="filterPage" onchange="filterV()" style="background:rgba(255,255,255,.05);border:1px solid rgba(147,51,234,.3);color:#fff;padding:.45rem .75rem;border-radius:8px;font-size:.8rem;">
                ...
                <button onclick="clearVis()" style="background:rgba(239,68,68,.15);border:1px solid rgba(239,68,68,.3);color:#ef4444;padding:.45rem .9rem;border-radius:8px;cursor:pointer;font-size:.8rem;font-weight:700;">🗑️ Effacer</button>
            </div>
        </div>
        <div id="vList">
        ${pubVisitors.length === 0
            ? '<div class="card" style="text-align:center;padding:2rem;color:var(--text-muted);">🌐 Aucun visiteur tracé pour l\'instant</div>'
            : pubVisitors.map(renderVisitor).join('')}
        </div>
        ${(db.publicVisitors||[]).length > 60 ? '<div style="text-align:center;color:var(--text-muted);font-size:.8rem;margin-top:.5rem;">... et '+((db.publicVisitors||[]).length-60)+' autres visiteurs</div>' : ''}
    </div>

    <script>
    var _vis=${JSON.stringify((db.publicVisitors||[]).slice(0,60))};
    function filterV(){
        var f=document.getElementById('filterPage').value;
        var arr=f?_vis.filter(function(v){return v.path===f;}):_vis;
        var cols={'/'  :'#22c55e','/candidature':'#a855f7','/boutique':'#f59e0b','/guide':'#3b82f6','/contact':'#ec4899','/palmares':'#f97316','/recrutement':'#14b8a6','/tactique':'#8b5cf6'};
        document.getElementById('vList').innerHTML=arr.length===0
            ?'<div class="card" style="text-align:center;padding:2rem;color:var(--text-muted);">Aucun visiteur sur cette page</div>'
            :arr.map(function(v){
                var geo=v.geo||{},flag=geo.emoji||'🌍',loc=(geo.city&&geo.country?geo.city+', '+geo.country:'Inconnu'),isp=geo.isp||'';
                var vpn=geo.vpn?'<span style="color:#f59e0b;font-size:.7rem;">⚠️ VPN</span>':'';
                var prx=geo.proxy?'<span style="color:#ef4444;font-size:.7rem;">🔴 PROXY</span>':'';
                var col=cols[v.path]||'#6b7280',dt=new Date(v.ts).toLocaleString('fr-FR');
                return '<div style="background:rgba(10,10,20,.7);border:1px solid rgba(147,51,234,.15);border-left:3px solid '+col+';border-radius:8px;padding:.75rem 1rem;margin-bottom:.5rem;">'
                    +'<div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:.3rem;"><span style="color:'+col+';font-weight:700;font-size:.875rem;">'+v.path+'</span><span style="font-size:.7rem;color:var(--text-muted);">'+dt+'</span></div>'
                    +'<div style="margin-top:.4rem;display:flex;flex-wrap:wrap;gap:.5rem;font-size:.75rem;color:var(--text-muted);">'
                    +'<span>🖥 '+(v.device||'?')+'</span><span>🌐 '+(v.browser||'?')+'</span><span>💿 '+(v.os||'?')+'</span>'
                    +'<span>🔌 <code style="background:rgba(255,255,255,.07);padding:.1rem .3rem;border-radius:4px;font-size:.7rem;">'+(v.ip||'?')+'</code> '+vpn+prx+'</span>'
                    +'<span>'+flag+' '+loc+(isp?' · '+isp:'')+'</span>'
                    +(v.referer?'<span>↩ '+v.referer.slice(0,50)+'</span>':'')
                    +'</div></div>';
            }).join('');
    }
    async function clearSec(){
        if(!confirm('Effacer tous les événements sécurité ?'))return;
        var r=await fetch('/api/security-events/clear',{method:'POST',headers:{'Content-Type':'application/json'}});
        if((await r.json()).success)location.reload();
    }
    async function clearVis(){
        if(!confirm('Effacer tous les visiteurs publics ?'))return;
        var r=await fetch('/api/public-visitors/clear',{method:'POST',headers:{'Content-Type':'application/json'}});
        if((await r.json()).success)location.reload();
    }
...
app.post('/api/screenshot-attempt', isAuthenticated, async (req, res) => {
    const { reason, ts } = req.body || {};
    const user = req.session.user;
    const ip   = getClientIP(req);
    const ci   = getClientInfo(req);
    const LABELS = {
        'getDisplayMedia:blocked': '📹 Partage ecran (OBS/Discord/Zoom)',
        'getUserMedia:blocked':    '📹 Capture video bloquee',
        'keydown:PrintScreen':     '📸 PrintScreen (keydown)',
        'keyup:PrintScreen':       '📸 PrintScreen (keyup)',
        'keydown:Ctrl+P':          '🖨 Impression / Export PDF',
        'keydown:Mac-Screenshot-3':'📸 macOS Cmd+Shift+3',
        'keydown:Mac-Screenshot-4':'📸 macOS Cmd+Shift+4',
        'keydown:Mac-Screenshot-5':'📸 macOS Cmd+Shift+5',
        'touch:3fingers':          '📱 Geste 3 doigts mobile',
        'mobile:window-blur':      '📱 Bouton screenshot mobile',
    };
    const label = LABELS[reason] || ('⚠️ ' + (reason || 'Inconnu'));
    addLog('📸 Tentative Capture', user.username, reason || 'unknown', { reason, label, ua: req.headers['user-agent'] || '', clientTs: ts || '' }, ip, ci);
    try {
        const geo = (ip && ip !== '127.0.0.1' && ip !== 'unknown') ? await getGeoIP(ip) : null;
        const db2 = readDB();
        if (!db2.securityEvents) db2.securityEvents = [];
        db2.securityEvents.unshift({
            id: 'sec_' + Date.now(), type: 'capture', label, rawReason: reason || 'unknown',
            username: user.username, role: user.role || user.accountType || '?',
            ip, device: ci.device||'', browser: ci.browser||'', os: ci.os||'',
            ua: req.headers['user-agent']||'', geo: geo||null,
            ts: new Date().toISOString(), clientTs: ts||null,
        });
        if (db2.securityEvents.length > 500) db2.securityEvents = db2.securityEvents.slice(0, 500);
        writeDB(db2);
    } catch(e) { console.error('[SecEvent]', e.message); }
    res.json({ received: true });
});

// Route : evenement securite generique (DevTools, iframe, etc.)
app.post('/api/security-event', isAuthenticated, async (req, res) => {
    const { reason, label, ts } = req.body || {};
    const user = req.session.user;
    const ip   = getClientIP(req);
    const ci   = getClientInfo(req);
    addLog('🛡️ Evenement Securite', user.username, reason || 'unknown', { reason, label, clientTs: ts }, ip, ci);
    try {
        const geo = (ip && ip !== '127.0.0.1' && ip !== 'unknown') ? await getGeoIP(ip) : null;
        const db2 = readDB();
        if (!db2.securityEvents) db2.securityEvents = [];
        db2.securityEvents.unshift({
            id: 'sec_' + Date.now(), type: 'security',
            label: label || ('⚠️ ' + (reason || 'Inconnu')), rawReason: reason || 'unknown',
            username: user.username, role: user.role || user.accountType || '?',
            ip, device: ci.device||'', browser: ci.browser||'', os: ci.os||'',
            ua: req.headers['user-agent']||'', geo: geo||null,
            ts: new Date().toISOString(), clientTs: ts||null,
...
// Route : effacer security events
app.post('/api/security-events/clear', isAuthenticated, hasRole('owner'), (req, res) => {
    if (req.session.user.username !== 'xywez') return res.status(403).json({ error: 'Reserve a Xywez' });
    const db2 = readDB(); db2.securityEvents = []; writeDB(db2);
    addLog('🗑 Security Events effaces', req.session.user.username, 'securityEvents', {}, getClientIP(req));
    res.json({ success: true });
});

// Route : effacer visiteurs publics
app.post('/api/public-visitors/clear', isAuthenticated, hasRole('owner'), (req, res) => {
    if (req.session.user.username !== 'xywez') return res.status(403).json({ error: 'Reserve a Xywez' });
    const db2 = readDB(); db2.publicVisitors = []; writeDB(db2);
    addLog('🗑 Visiteurs publics effaces', req.session.user.username, 'publicVisitors', {}, getClientIP(req));
    res.json({ success: true });
});
...
<div class="confirm-box" onclick="tog(event,1)">
<input type="checkbox" id="c1" onchange="chk()">
<label for="c1" onclick="event.stopPropagation()">J'ai lu, compris et j'accepte <strong>toutes les règles</strong> du panel FTY Club Pro. Je suis conscient(e) que toutes mes actions sont enregistrées.</label>
</div>
<div class="confirm-box" onclick="tog(event,2)">
<input type="checkbox" id="c2" onchange="chk()">
<label for="c2" onclick="event.stopPropagation()">Je m'engage à respecter la <strong>confidentialité totale</strong> des informations et à ne jamais partager mes accès.</label>
</div>
<div class="confirm-box" onclick="tog(event,3)">
<input type="checkbox" id="c3" onchange="chk()">
<label for="c3" onclick="event.stopPropagation()">Je comprends que tout abus entraînera une <strong>sanction immédiate</strong> (suspension/ban panel + Discord) sans appel possible.</label>
</div>
...
function tog(e,n){if(e.target.tagName==='INPUT'||e.target.tagName==='LABEL')return;var c=document.getElementById('c'+n);c.checked=!c.checked;chk();}
